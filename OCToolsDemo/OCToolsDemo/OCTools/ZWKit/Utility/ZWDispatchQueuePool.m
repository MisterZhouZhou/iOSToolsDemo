...
static ZWDispatchContext *ZWDispatchContextCreate(const char *name,
                                                  uint32_t queueCount,
                                                  NSQualityOfService qos) {
    ZWDispatchContext *context = calloc(1, sizeof(ZWDispatchContext));
...
static void ZWDispatchContextRelease(ZWDispatchContext *context) {
    if (!context) return;
    if (context->queues) {
...
static dispatch_queue_t ZWDispatchContextGetQueue(ZWDispatchContext *context) {
...
static ZWDispatchContext *ZWDispatchContextGetForQOS(NSQualityOfService qos) {
    static ZWDispatchContext *context[5] = {0};
    switch (qos) {
        case NSQualityOfServiceUserInteractive: {
            ...
            dispatch_once(&onceToken, ^{
                ...
                context[0] = ZWDispatchContextCreate("com.ibireme.yykit.user-interactive", count, qos);
            });
            return context[0];
        } break;
        case NSQualityOfServiceUserInitiated: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[1] = ZWDispatchContextCreate("com.ibireme.yykit.user-initiated", count, qos);
            });
            return context[1];
        } break;
        case NSQualityOfServiceUtility: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[2] = ZWDispatchContextCreate("com.ibireme.yykit.utility", count, qos);
            });
            return context[2];
        } break;
        case NSQualityOfServiceBackground: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[3] = ZWDispatchContextCreate("com.ibireme.yykit.background", count, qos);
            });
            return context[3];
        } break;
        ...
        default: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                context[4] = ZWDispatchContextCreate("com.ibireme.yykit.default", count, qos);
            });
            return context[4];
        } break;
    }
...
@implementation ZWDispatchQueuePool {
@public
    ZWDispatchContext *_context;
}

- (void)dealloc {
    if (_context) {
        ZWDispatchContextRelease(_context);
        _context = NULL;
    }
}

- (instancetype)initWithContext:(ZWDispatchContext *)context {
    self = [super init];
...
- (instancetype)initWithName:(NSString *)name queueCount:(NSUInteger)queueCount qos:(NSQualityOfService)qos {
    ...
    self = [super init];
    _context = ZWDispatchContextCreate(name.UTF8String, (uint32_t)queueCount, qos);
...
- (dispatch_queue_t)queue {
    return ZWDispatchContextGetQueue(_context);
}

+ (instancetype)defaultPoolForQOS:(NSQualityOfService)qos {
    switch (qos) {
        case NSQualityOfServiceUserInteractive: {
            static ZWDispatchQueuePool *pool;
            static dispatch_once_t onceToken;
            dispatch_once(&onceToken, ^{
                pool = [[ZWDispatchQueuePool alloc] initWithContext:ZWDispatchContextGetForQOS(qos)];
            });
            return pool;
        } break;
        case NSQualityOfServiceUserInitiated: {
        ...
        default: {
            ...
            static dispatch_once_t onceToken;
            dispatch_once(&onceToken, ^{
                pool = [[ZWDispatchQueuePool alloc] initWithContext:ZWDispatchContextGetForQOS(NSQualityOfServiceDefault)];
            });
            return pool;
        } break;
    }
...
dispatch_queue_t ZWDispatchQueueGetForQOS(NSQualityOfService qos) {
    return ZWDispatchContextGetQueue(ZWDispatchContextGetForQOS(qos));
}
