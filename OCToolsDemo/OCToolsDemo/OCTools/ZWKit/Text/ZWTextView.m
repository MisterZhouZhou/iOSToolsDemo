...
#import "ZWTextEffectWindow.h"
#import "ZWTextKeyboardManager.h"
...
@interface ZWTextView () <UIScrollViewDelegate, UIAlertViewDelegate, ZWTextDebugTarget, ZWTextKeyboardObserver> {
    
    ZWTextRange *_selectedTextRange; /// nonnull
    ZWTextRange *_markedTextRange;
    
    __weak id<ZWTextViewDelegate> _outerDelegate;
    ...
    ZWTextContainer *_innerContainer; ///< nonnull, inner text container
    ZWTextLayout *_innerLayout; ///< inner text layout, the text in this layout is longer than `_innerText` by appending '\n'
    
    ZWTextContainerView *_containerView; ///< nonnull
    ZWTextSelectionView *_selectionView; ///< nonnull
    ZWTextMagnifier *_magnifierCaret; ///< nonnull
    ZWTextMagnifier *_magnifierRanged; ///< nonnull
    ...
    ZWTextHighlight *_highlight; ///< highlight attribute in `_highlightRange`
    ZWTextLayout *_highlightLayout; ///< when _state.showingHighlight=YES, this layout should be displayed
    ZWTextRange *_trackingRange; ///< the range in _innerLayout, may out of _innerText.
...
- (void)_commitUpdate {
    ...
    [[ZWTransaction transactionWithTarget:self selector:@selector(_updateIfNeeded)] commit];
#else
    [self _update];
#endif
}
...
- (void)_updateLayout {
    ...
    [text removeAttribute:ZWTextBorderAttributeName range:NSMakeRange(_innerText.length, 1)];
    [text removeAttribute:ZWTextBackgroundBorderAttributeName range:NSMakeRange(_innerText.length, 1)];
    ...
    _innerLayout = [ZWTextLayout layoutWithContainer:_innerContainer text:text];
...
- (void)_updateSelectionView {
    ...
    [[ZWTextEffectWindow sharedWindow] hideSelectionDot:_selectionView];
    if (!_innerLayout) return;
    
    NSMutableArray *allRects = [NSMutableArray new];
    BOOL containsDot = NO;
    
    ZWTextRange *selectedRange = _selectedTextRange;
    ...
    [allRects enumerateObjectsUsingBlock:^(ZWTextSelectionRect *rect, NSUInteger idx, BOOL *stop) {
    ...
    if (!_state.firstShowDot && containsDot) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.02 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [[ZWTextEffectWindow sharedWindow] showSelectionDot:_selectionView];
        });
    }
    [[ZWTextEffectWindow sharedWindow] showSelectionDot:_selectionView];
...
- (void)_commitPlaceholderUpdate {
    ...
    [[ZWTransaction transactionWithTarget:self selector:@selector(_updatePlaceholderIfNeeded)] commit];
#else
...
- (void)_updatePlaceholder {
    ...
    if (_placeholderAttributedText.length > 0) {
        ZWTextContainer *container = _innerContainer.copy;
        container.size = self.bounds.size;
        container.truncationType = ZWTextTruncationTypeEnd;
        container.truncationToken = nil;
        ZWTextLayout *layout = [ZWTextLayout layoutWithContainer:container text:_placeholderAttributedText];
...
- (void)_updateTextRangeByTrackingCaret {
    ...
    ZWTextPosition *newPos = [_innerLayout closestPositionToPoint:trackingPoint];
    if (newPos) {
        ...
        ZWTextRange *newRange = [ZWTextRange rangeWithRange:NSMakeRange(newPos.offset, 0) affinity:newPos.affinity];
...
- (void)_updateTextRangeByTrackingGrabber {
    ...
    ZWTextPosition *position = [_innerLayout positionForPoint:magPoint
    ...
    if (position) {
        ...
        if ((NSUInteger)position.offset > _innerText.length) {
            position = [ZWTextPosition positionWithOffset:_innerText.length];
        }
        ZWTextRange *newRange = [ZWTextRange rangeWithStart:(isStart ? position : _selectedTextRange.start)
...
- (void)_updateTextRangeByTrackingPreSelect {
    if (!_state.trackingTouch) return;
    ZWTextRange *newRange = [self _getClosestTokenRangeAtPoint:_trackingPoint];
    _trackingRange = newRange;
}
...
- (void)_showMagnifierCaret {
    ...
    if (_state.showingMagnifierRanged) {
        ...
        [[ZWTextEffectWindow sharedWindow] hideMagnifier:_magnifierRanged];
    }
    ...
    if (!_state.showingMagnifierCaret) {
        ...
        [[ZWTextEffectWindow sharedWindow] showMagnifier:_magnifierCaret];
    } else {
        [[ZWTextEffectWindow sharedWindow] moveMagnifier:_magnifierCaret];
    }
}
...
- (void)_showMagnifierRanged {
    ...
    if (_state.showingMagnifierCaret) {
        ...
        [[ZWTextEffectWindow sharedWindow] hideMagnifier:_magnifierCaret];
    }
    ...
    if (lineIndex < _innerLayout.lines.count) {
        ZWTextLine *line = _innerLayout.lines[lineIndex];
        CGRect lineRect = [self _convertRectFromLayout:line.bounds];
        if (_verticalForm) {
            magPoint.x = ZW_CLAMP(magPoint.x, CGRectGetMinX(lineRect), CGRectGetMaxX(lineRect));
        } else {
            magPoint.y = ZW_CLAMP(magPoint.y, CGRectGetMinY(lineRect), CGRectGetMaxY(lineRect));
        }
        ...
        if (!_state.showingMagnifierRanged) {
            ...
            [[ZWTextEffectWindow sharedWindow] showMagnifier:_magnifierRanged];
        } else {
            [[ZWTextEffectWindow sharedWindow] moveMagnifier:_magnifierRanged];
        }
    }
}
...
- (void)_showMenu {
    ...
    if (_selectionView.caretVisible) {
        ...
        ZWTextSelectionRect *sRect = _selectionView.selectionRects.firstObject;
        rect = sRect.rect;
        ...
        ZWTextKeyboardManager *mgr = [ZWTextKeyboardManager defaultManager];
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        _highlightLayout = [ZWTextLayout layoutWithContainer:_innerContainer text:hiText];
...
- (void)_scrollRangeToVisible:(ZWTextRange *)range {
    if (!range) return;
...
- (void)_keyboardChanged {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if ([ZWTextKeyboardManager defaultManager].keyboardVisible) {
            [self _scrollRangeToVisible:_selectedTextRange];
        } else {
...
- (void)_trackDidLongPress {
    ...
    if (_state.showingHighlight) {
        ...
        if (_highlight.longPressAction) {
            ...
            CGRect rect = [_innerLayout rectForRange:[ZWTextRange rangeWithRange:_highlightRange]];
            rect = [self _convertRectFromLayout:rect];
        ...
        if (_state.trackingTouch) {
            if (_state.trackingGrabber) {
                ...
                if (newPos) {
                    ...
                    _trackingRange = [ZWTextRange rangeWithRange:NSMakeRange(newPos.offset, 0) affinity:newPos.affinity];
                    [self _updateSelectionView];
                }
                [self _hideMenu];
...
- (void)_fixSelectionDot {
    ...
    CGPoint origin = [self convertPoint:CGPointZero toViewOrWindow:[ZWTextEffectWindow sharedWindow]];
...
- (ZWTextRange *)_getClosestTokenRangeAtPosition:(ZWTextPosition *)position {
    position = [self _correctedTextPosition:position];
    if (!position) return nil;
    ZWTextRange *range = nil;
    if (_tokenizer) {
    ...
    if (!range || range.asRange.length == 0) {
        ...
        ZWTextRange *extStart = [_innerLayout textRangeByExtendingPosition:range.start];
        ZWTextRange *extEnd = [_innerLayout textRangeByExtendingPosition:range.end];
        if (extStart && extEnd) {
            NSArray *arr = [@[extStart.start, extStart.end, extEnd.start, extEnd.end] sortedArrayUsingSelector:@selector(compare:)];
            range = [ZWTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    }
    
    range = [self _correctedTextRange:range];
    if (range.asRange.length == 0) {
        range = [ZWTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    }
...
/// Try to get the character range/position with word granularity from the tokenizer.
- (ZWTextRange *)_getClosestTokenRangeAtPoint:(CGPoint)point {
    point = [self _convertPointToLayout:point];
    ZWTextRange *touchRange = [_innerLayout closestTextRangeAtPoint:point];
    touchRange = [self _correctedTextRange:touchRange];
    
    if (_tokenizer && touchRange) {
        ZWTextRange *encEnd = (id)[_tokenizer rangeEnclosingPosition:touchRange.end withGranularity:UITextGranularityWord inDirection:UITextStorageDirectionBackward];
        ZWTextRange *encStart = (id)[_tokenizer rangeEnclosingPosition:touchRange.start withGranularity:UITextGranularityWord inDirection:UITextStorageDirectionForward];
        if (encEnd && encStart) {
            NSArray *arr = [@[encEnd.start, encEnd.end, encStart.start, encStart.end] sortedArrayUsingSelector:@selector(compare:)];
            touchRange = [ZWTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    }
    
    if (touchRange) {
        ZWTextRange *extStart = [_innerLayout textRangeByExtendingPosition:touchRange.start];
        ZWTextRange *extEnd = [_innerLayout textRangeByExtendingPosition:touchRange.end];
        if (extStart && extEnd) {
            NSArray *arr = [@[extStart.start, extStart.end, extEnd.start, extEnd.end] sortedArrayUsingSelector:@selector(compare:)];
            touchRange = [ZWTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    ...
    if (!touchRange) touchRange = [ZWTextRange defaultRange];
    
    if (_innerText.length && touchRange.asRange.length == 0) {
        touchRange = [ZWTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    }
...
- (ZWTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    if (!_highlightable || !_innerLayout.containsHighlight) return nil;
    point = [self _convertPointToLayout:point];
    ZWTextRange *textRange = [_innerLayout textRangeAtPoint:point];
    ...
    ZWTextHighlight *highlight = [text attribute:ZWTextHighlightAttributeName
                                         atIndex:startIndex
...
- (CGFloat)_getMagnifierRangedOffset {
    ...
    ZWTextPosition *position = [_innerLayout closestPositionToPoint:magPoint];
    NSUInteger lineIndex = [_innerLayout lineIndexForPosition:position];
    if (lineIndex < _innerLayout.lines.count) {
        ZWTextLine *line = _innerLayout.lines[lineIndex];
        if (_verticalForm) {
            magPoint.x = ZW_CLAMP(magPoint.x, line.left, line.right);
            return magPoint.x - line.position.x + kMagnifierRangedPopoverOffset;
        } else {
            magPoint.y = ZW_CLAMP(magPoint.y, line.top, line.bottom);
...
- (BOOL)_isTextPositionValid:(ZWTextPosition *)position {
    if (!position) return NO;
    if (position.offset < 0) return NO;
    if (position.offset > _innerText.length) return NO;
    if (position.offset == 0 && position.affinity == ZWTextAffinityBackward) return NO;
    if (position.offset == _innerText.length && position.affinity == ZWTextAffinityBackward) return NO;
    return YES;
}

/// Whether the range is valid (not out of bounds).
- (BOOL)_isTextRangeValid:(ZWTextRange *)range {
...
- (ZWTextPosition *)_correctedTextPosition:(ZWTextPosition *)position {
    if (!position) return nil;
    if ([self _isTextPositionValid:position]) return position;
    if (position.offset < 0) {
        return [ZWTextPosition positionWithOffset:0];
    }
    if (position.offset > _innerText.length) {
        return [ZWTextPosition positionWithOffset:_innerText.length];
    }
    if (position.offset == 0 && position.affinity == ZWTextAffinityBackward) {
        return [ZWTextPosition positionWithOffset:position.offset];
    }
    if (position.offset == _innerText.length && position.affinity == ZWTextAffinityBackward) {
        return [ZWTextPosition positionWithOffset:position.offset];
    }
    return position;
}
...
- (ZWTextRange *)_correctedTextRange:(ZWTextRange *)range {
    if (!range) return nil;
    if ([self _isTextRangeValid:range]) return range;
    ZWTextPosition *start = [self _correctedTextPosition:range.start];
    ZWTextPosition *end = [self _correctedTextPosition:range.end];
    return [ZWTextRange rangeWithStart:start end:end];
}
...
- (void)_replaceRange:(ZWTextRange *)range withText:(NSString *)text notifyToDelegate:(BOOL)notify{
    if (NSEqualRanges(range.asRange, _selectedTextRange.asRange)) {
        ...
        _selectedTextRange = [ZWTextRange rangeWithRange:newRange];
        ...
        if (range.asRange.length != text.length) {
            ...
            if (unionRange.length == 0) {
                ...
                _selectedTextRange = [ZWTextRange rangeWithRange:NSMakeRange(range.start.offset + text.length, 0)];
...
- (void)_updateAttributesHolder {
    if (_innerText.length > 0) {
        ...
        [_typingAttributesHolder removeAttribute:ZWTextBorderAttributeName range:NSMakeRange(0, _typingAttributesHolder.length)];
        [_typingAttributesHolder removeAttribute:ZWTextBackgroundBorderAttributeName range:NSMakeRange(0, _typingAttributesHolder.length)];
    }
}
...
- (BOOL)_parseText {
    if (self.textParser) {
        ZWTextRange *oldTextRange = _selectedTextRange;
        NSRange newRange = _selectedTextRange.asRange;
        ...
        ZWTextRange *newTextRange = [ZWTextRange rangeWithRange:newRange];
        ...
        switch (result.resultType) {
            ...
            case NSTextCheckingTypePhoneNumber: {
                ...
                if (_highlightTextAttributes.count) {
                    ZWTextHighlight *highlight = [ZWTextHighlight highlightWithAttributes:_highlightTextAttributes];
...
- (void)_setTextParser:(id<ZWTextParser>)textParser {
...
- (void)_setLinePositionModifier:(id<ZWTextLinePositionModifier>)linePositionModifier {
...
- (void)_initTextView {
    ...
    // UITextInput
    _selectedTextRange = [ZWTextRange defaultRange];
    ...
    _innerContainer = [ZWTextContainer new];
    ...
    ZWTextHighlight *highlight = [ZWTextHighlight new];
    ZWTextBorder * border = [ZWTextBorder new];
    ...
    _containerView = [ZWTextContainerView new];
    _containerView.hostView = self;
    
    _selectionView = [ZWTextSelectionView new];
    _selectionView.userInteractionEnabled = NO;
    _selectionView.hostView = self;
    _selectionView.color = [self _defaultTintColor];
    
    _magnifierCaret = [ZWTextMagnifier magnifierWithType:ZWTextMagnifierTypeCaret];
    _magnifierCaret.hostView = _containerView;
    _magnifierRanged = [ZWTextMagnifier magnifierWithType:ZWTextMagnifierTypeRanged];
    ...
    self.debugOption = [ZWTextDebugOption sharedDebugOption];
    [ZWTextDebugOption addDebugTarget:self];
    ...
    [[ZWTextKeyboardManager defaultManager] addObserver:self];
...
- (void)dealloc {
    ...
    [[ZWTextKeyboardManager defaultManager] removeObserver:self];
    
    [[ZWTextEffectWindow sharedWindow] hideSelectionDot:_selectionView];
    [[ZWTextEffectWindow sharedWindow] hideMagnifier:_magnifierCaret];
    [[ZWTextEffectWindow sharedWindow] hideMagnifier:_magnifierRanged];
    
    [ZWTextDebugOption removeDebugTarget:self];
...
- (void)scrollRangeToVisible:(NSRange)range {
    ZWTextRange *textRange = [ZWTextRange rangeWithRange:range];
    textRange = [self _correctedTextRange:textRange];
...
- (void)setText:(NSString *)text {
    ...
    [self replaceRange:[ZWTextRange rangeWithRange:NSMakeRange(0, _innerText.length)] withText:text];
}
...
- (void)setTextParser:(id<ZWTextParser>)textParser {
    if (_textParser == textParser || [_textParser isEqual:textParser]) return;
    [self _setTextParser:textParser];
    if (textParser && _text.length) {
        [self replaceRange:[ZWTextRange rangeWithRange:NSMakeRange(0, _text.length)] withText:_text];
    }
    [self _resetUndoAndRedoStack];
    [self _commitUpdate];
}
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    ...
    if (text.length == 0) {
        [self replaceRange:[ZWTextRange rangeWithRange:NSMakeRange(0, _innerText.length)] withText:@""];
        return;
    }
    ...
    _innerText = text;
    [self _parseText];
    _selectedTextRange = [ZWTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    [_inputDelegate textDidChange:self];
    [_inputDelegate selectionDidChange:self];
...
- (void)setSelectedRange:(NSRange)selectedRange {
    ...
    ZWTextRange *range = [ZWTextRange rangeWithRange:selectedRange];
    range = [self _correctedTextRange:range];
    [self _endTouchTracking];
...
- (ZWTextDebugOption *)debugOption {
    return _containerView.debugOption;
}

- (ZWTextLayout *)textLayout {
...
- (CGSize)sizeThatFits:(CGSize)size {
    if (!_verticalForm && size.width <= 0) size.width = ZWTextContainerMaxSize.width;
    if (_verticalForm && size.height <= 0) size.height = ZWTextContainerMaxSize.height;
    ...
    if (!_verticalForm) {
        size.height = ZWTextContainerMaxSize.height;
    } else {
        size.width = ZWTextContainerMaxSize.width;
    }
    
    ZWTextContainer *container = [_innerContainer copy];
    container.size = size;
    
    ZWTextLayout *layout = [ZWTextLayout layoutWithContainer:container text:_innerText];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            ...
            ZWTextHighlight *highlight = [self _getHighlightAtPoint:_trackingPoint range:NULL];
            ...
            if (_state.trackingCaret) {
                ...
                if (!_state.touchMoved) {
                    if (_state.selectedWithoutEdit) {
                        ...
                        if (self.isFirstResponder) {
                            ZWTextRange *_oldRange = _trackingRange;
                            [self _updateTextRangeByTrackingCaret];
...
- (void)paste:(id)sender {
    ...
    if (!atr && _allowsPasteImage) {
        UIImage *img = nil;
        if (p.GIFData) {
            img = [ZWImage imageWithData:p.GIFData scale:kScreenScale];
        }
        if (!img && p.PNGData) {
            img = [ZWImage imageWithData:p.PNGData scale:kScreenScale];
        }
        if (!img && p.WEBPData) {
            img = [ZWImage imageWithData:p.WEBPData scale:kScreenScale];
        }
        if (!img) {
            img = p.image;
        }
        ...
        if (img && img.size.width > 1 && img.size.height > 1) {
            id content = img;
            if ([img conformsToProtocol:@protocol(ZWAnimatedImage)]) {
                id<ZWAnimatedImage> ani = (id)img;
                if (ani.animatedImageFrameCount > 1) {
                    ZWAnimatedImageView *aniView = [[ZWAnimatedImageView alloc] initWithImage:img];
                    if (aniView) {
                        content = aniView;
                    }
                }
    ...
    if (atr) {
        ...
        ZWTextPosition *pos = [self _correctedTextPosition:[ZWTextPosition positionWithOffset:endPosition]];
        ZWTextRange *range = [_innerLayout textRangeByExtendingPosition:pos];
        range = [self _correctedTextRange:range];
        if (range) {
...
- (void)select:(id)sender {
    ...
    ZWTextRange *newRange = [self _getClosestTokenRangeAtPosition:_selectedTextRange.start];
...
- (id<ZWTextViewDelegate>)delegate {
    return _outerDelegate;
}

- (void)setDelegate:(id<ZWTextViewDelegate>)delegate {
...
- (void)keyboardChangedWithTransition:(ZWTextKeyboardTransition)transition {
    [self _keyboardChanged];
}
...
- (void)deleteBackward {
    ...
    if (!_state.deleteConfirm && range.length == 0 && range.location > 0) {
        ...
        ZWTextBinding *binding = [_innerText attribute:ZWTextBindingAttributeName atIndex:range.location - 1 longestEffectiveRange:&effectiveRange inRange:NSMakeRange(0, _innerText.length)];
        if (binding && binding.deleteConfirm) {
            _state.deleteConfirm = YES;
            [_inputDelegate selectionWillChange:self];
            _selectedTextRange = [ZWTextRange rangeWithRange:effectiveRange];
            _selectedTextRange = [self _correctedTextRange:_selectedTextRange];
            [_inputDelegate selectionDidChange:self];
    ...
    _state.deleteConfirm = NO;
    if (range.length == 0) {
        ZWTextRange *extendRange = [_innerLayout textRangeByExtendingPosition:_selectedTextRange.end inDirection:UITextLayoutDirectionLeft offset:1];
    ...
    [self replaceRange:[ZWTextRange rangeWithRange:range] withText:@""];
}
...
- (void)setSelectedTextRange:(ZWTextRange *)selectedTextRange {
...
- (void)setMarkedText:(NSString *)markedText selectedRange:(NSRange)selectedRange {
    ...
    if (_markedTextRange == nil) {
        _markedTextRange = [ZWTextRange rangeWithRange:NSMakeRange(_selectedTextRange.end.offset, markedText.length)];
        [_innerText replaceCharactersInRange:NSMakeRange(_selectedTextRange.end.offset, 0) withString:markedText];
        _selectedTextRange = [ZWTextRange rangeWithRange:NSMakeRange(_selectedTextRange.start.offset + selectedRange.location, selectedRange.length)];
    } else {
        ...
        _markedTextRange = [ZWTextRange rangeWithRange:NSMakeRange(_markedTextRange.start.offset, markedText.length)];
        _selectedTextRange = [ZWTextRange rangeWithRange:NSMakeRange(_markedTextRange.start.offset + selectedRange.location, selectedRange.length)];
    }
...
- (void)replaceRange:(ZWTextRange *)range withText:(NSString *)text {
    if (!range) return;
    if (!text) text = @"";
...
- (void)setBaseWritingDirection:(UITextWritingDirection)writingDirection forRange:(ZWTextRange *)range {
    if (!range) return;
    range = [self _correctedTextRange:range];
...
- (NSString *)textInRange:(ZWTextRange *)range {
    range = [self _correctedTextRange:range];
    if (!range) return @"";
    return [_innerText.string substringWithRange:range.asRange];
}

- (UITextWritingDirection)baseWritingDirectionForPosition:(ZWTextPosition *)position inDirection:(UITextStorageDirection)direction {
    [self _updateIfNeeded];
    position = [self _correctedTextPosition:position];
...
- (ZWTextPosition *)beginningOfDocument {
    return [ZWTextPosition positionWithOffset:0];
}

- (ZWTextPosition *)endOfDocument {
    return [ZWTextPosition positionWithOffset:_innerText.length];
}

- (ZWTextPosition *)positionFromPosition:(ZWTextPosition *)position offset:(NSInteger)offset {
    ...
    if (newLocation != 0 && newLocation != _innerText.length) {
        // fix emoji
        [self _updateIfNeeded];
        ZWTextRange *extendRange = [_innerLayout textRangeByExtendingPosition:[ZWTextPosition positionWithOffset:newLocation]];
    ...
    ZWTextPosition *p = [ZWTextPosition positionWithOffset:newLocation];
    return [self _correctedTextPosition:p];
}

- (ZWTextPosition *)positionFromPosition:(ZWTextPosition *)position inDirection:(UITextLayoutDirection)direction offset:(NSInteger)offset {
    [self _updateIfNeeded];
    ZWTextRange *range = [_innerLayout textRangeByExtendingPosition:position inDirection:direction offset:offset];
    ...
    ZWTextPosition *newPosition = forward ? range.end : range.start;
    if (newPosition.offset > _innerText.length) {
        newPosition = [ZWTextPosition positionWithOffset:_innerText.length affinity:ZWTextAffinityBackward];
    }
    
    return [self _correctedTextPosition:newPosition];
}

- (ZWTextRange *)textRangeFromPosition:(ZWTextPosition *)fromPosition toPosition:(ZWTextPosition *)toPosition {
    return [ZWTextRange rangeWithStart:fromPosition end:toPosition];
}

- (NSComparisonResult)comparePosition:(ZWTextPosition *)position toPosition:(ZWTextPosition *)other {
    return [position compare:other];
}

- (NSInteger)offsetFromPosition:(ZWTextPosition *)from toPosition:(ZWTextPosition *)toPosition {
    return toPosition.offset - from.offset;
}

- (ZWTextPosition *)positionWithinRange:(ZWTextRange *)range farthestInDirection:(UITextLayoutDirection)direction {
    NSRange nsRange = range.asRange;
    if (direction == UITextLayoutDirectionLeft | direction == UITextLayoutDirectionUp) {
        return [ZWTextPosition positionWithOffset:nsRange.location];
    } else {
        return [ZWTextPosition positionWithOffset:nsRange.location + nsRange.length affinity:ZWTextAffinityBackward];
    }
}

- (ZWTextRange *)characterRangeByExtendingPosition:(ZWTextPosition *)position inDirection:(UITextLayoutDirection)direction {
    [self _updateIfNeeded];
    ZWTextRange *range = [_innerLayout textRangeByExtendingPosition:position inDirection:direction offset:1];
    return [self _correctedTextRange:range];
}
...
- (ZWTextPosition *)closestPositionToPoint:(CGPoint)point {
    [self _updateIfNeeded];
    point = [self _convertPointToLayout:point];
    ZWTextPosition *position = [_innerLayout closestPositionToPoint:point];
    return [self _correctedTextPosition:position];
}

- (ZWTextPosition *)closestPositionToPoint:(CGPoint)point withinRange:(ZWTextRange *)range {
    ZWTextPosition *pos = (id)[self closestPositionToPoint:point];
    if (!pos) return nil;
...
- (ZWTextRange *)characterRangeAtPoint:(CGPoint)point {
    [self _updateIfNeeded];
    point = [self _convertPointToLayout:point];
    ZWTextRange *r = [_innerLayout closestTextRangeAtPoint:point];
...
- (NSArray *)selectionRectsForRange:(ZWTextRange *)range {
    ...
    [rects enumerateObjectsUsingBlock:^(ZWTextSelectionRect *rect, NSUInteger idx, BOOL *stop) {
        rect.rect = [self _convertRectFromLayout:rect.rect];
    }];
    return rects;
}
...
- (UITextStorageDirection)selectionAffinity {
    if (_selectedTextRange.end.affinity == ZWTextAffinityForward) {
...
- (void)setSelectionAffinity:(UITextStorageDirection)selectionAffinity {
    _selectedTextRange = [ZWTextRange rangeWithRange:_selectedTextRange.asRange affinity:selectionAffinity == UITextStorageDirectionForward ? ZWTextAffinityForward : ZWTextAffinityBackward];
    [self _updateSelectionView];
}

- (NSDictionary *)textStylingAtPosition:(ZWTextPosition *)position inDirection:(UITextStorageDirection)direction {
    if (!position) return nil;
...
- (ZWTextPosition *)positionWithinRange:(ZWTextRange *)range atCharacterOffset:(NSInteger)offset {
    if (!range) return nil;
    ...
    else return [ZWTextPosition positionWithOffset:offset];
}

- (NSInteger)characterOffsetOfPosition:(ZWTextPosition *)position withinRange:(ZWTextRange *)range {
...
@interface ZWTextView(IBInspectableProperties)
@end

@implementation ZWTextView(IBInspectableProperties)
...
- (void)setDebugEnabled_:(BOOL)enabled {
    if (!enabled) {
        ...
        ZWTextDebugOption *debugOption = [ZWTextDebugOption new];
