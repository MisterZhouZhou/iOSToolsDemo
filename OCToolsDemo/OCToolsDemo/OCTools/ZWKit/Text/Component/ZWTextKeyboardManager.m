...
static int _ZWTextKeyboardViewFrameObserverKey;

/// Observer for view's frame/bounds/center/transform
@interface _ZWTextKeyboardViewFrameObserver : NSObject
...
@implementation _ZWTextKeyboardViewFrameObserver {
...
- (void)addToKeyboardView:(UIView *)keyboardView {
    ...
    if (_keyboardView) {
        ...
        objc_setAssociatedObject(_keyboardView, &_ZWTextKeyboardViewFrameObserverKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _keyboardView = keyboardView;
    if (keyboardView) {
        [self addFrameObserver];
    }
    objc_setAssociatedObject(keyboardView, &_ZWTextKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
+ (instancetype)observerForView:(UIView *)keyboardView {
    ...
    return objc_getAssociatedObject(keyboardView, &_ZWTextKeyboardViewFrameObserverKey);
}
...
@implementation ZWTextKeyboardManager {
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"ZWTextKeyboardManager init error" reason:@"Use 'defaultManager' to get instance." userInfo:nil];
    return [super init];
}
...
- (void)_initFrameObserver {
    ...
    _ZWTextKeyboardViewFrameObserver *observer = [_ZWTextKeyboardViewFrameObserver observerForView:keyboardView];
    if (!observer) {
        observer = [_ZWTextKeyboardViewFrameObserver new];
...
+ (instancetype)defaultManager {
    static ZWTextKeyboardManager *mgr = nil;
...
- (void)addObserver:(id<ZWTextKeyboardObserver>)observer {
    if (!observer) return;
    [_observers addObject:observer];
}

- (void)removeObserver:(id<ZWTextKeyboardObserver>)observer {
    if (!observer) return;
...
- (void)_notifyAllObservers {
    ...
    ZWTextKeyboardTransition trans = {0};
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<ZWTextKeyboardObserver> observer in _observers.copy) {
