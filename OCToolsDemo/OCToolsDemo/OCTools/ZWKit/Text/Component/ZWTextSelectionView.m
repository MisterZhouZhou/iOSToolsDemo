...
@implementation ZWSelectionGrabberDot
...
@implementation ZWSelectionGrabber

- (instancetype) initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (!self) return nil;
    _dot = [[ZWSelectionGrabberDot alloc] initWithFrame:CGRectMake(0, 0, 10, 10)];
    return self;
}

- (void)setDotDirection:(ZWTextDirection)dotDirection {
    _dotDirection = dotDirection;
    [self addSubview:_dot];
    CGRect frame = _dot.frame;
    CGFloat ofs = 0.5;
    if (dotDirection == ZWTextDirectionTop) {
        frame.origin.y = -frame.size.height + ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == ZWTextDirectionRight) {
        frame.origin.x = self.bounds.size.width - ofs;
        frame.origin.y = (self.bounds.size.height - frame.size.height) / 2;
    } else if (dotDirection == ZWTextDirectionBottom) {
        frame.origin.y = self.bounds.size.height - ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == ZWTextDirectionLeft) {
...
- (CGRect)touchRect {
    ...
    if (_dotDirection == ZWTextDirectionTop) {
        insets.top = -kTouchDotExtend;
    } else if (_dotDirection == ZWTextDirectionRight) {
        insets.right = -kTouchDotExtend;
    } else if (_dotDirection == ZWTextDirectionBottom) {
        insets.bottom = -kTouchDotExtend;
    } else if (_dotDirection == ZWTextDirectionLeft) {
...
@interface ZWTextSelectionView ()
@property (nonatomic, strong) NSTimer *caretTimer;
@property (nonatomic, strong) UIView *caretView;
@property (nonatomic, strong) ZWSelectionGrabber *startGrabber;
@property (nonatomic, strong) ZWSelectionGrabber *endGrabber;
@property (nonatomic, strong) NSMutableArray *markViews;
@end

@implementation ZWTextSelectionView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _startGrabber = [ZWSelectionGrabber new];
    _startGrabber.dotDirection = ZWTextDirectionTop;
    _startGrabber.hidden = YES;
    _endGrabber = [ZWSelectionGrabber new];
    _endGrabber.dotDirection = ZWTextDirectionBottom;
...
- (void)_startBlinks {
    [_caretTimer invalidate];
    if (_caretVisible) {
        _caretTimer = [NSTimer timerWithTimeInterval:kBlinkDuration target:[ZWWeakProxy proxyWithTarget:self] selector:@selector(_doBlink) userInfo:nil repeats:YES];
...
- (void)setVerticalForm:(BOOL)verticalForm {
    if (_verticalForm != verticalForm) {
        ...
        self.startGrabber.dotDirection = verticalForm ? ZWTextDirectionRight : ZWTextDirectionTop;
        self.endGrabber.dotDirection = verticalForm ? ZWTextDirectionLeft : ZWTextDirectionBottom;
    }
}
...
- (void)setSelectionRects:(NSArray *)selectionRects {
    ...
    [selectionRects enumerateObjectsUsingBlock: ^(ZWTextSelectionRect *r, NSUInteger idx, BOOL *stop) {
        CGRect rect = r.rect;
...
- (BOOL)isSelectionRectsContainsPoint:(CGPoint)point {
    ...
    for (ZWTextSelectionRect *rect in _selectionRects) {
