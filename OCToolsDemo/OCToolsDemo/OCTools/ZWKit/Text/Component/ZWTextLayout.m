...
const CGSize ZWTextContainerMaxSize = (CGSize){0x100000, 0x100000};
...
static inline CGSize ZWTextClipCGSize(CGSize size) {
    if (size.width > ZWTextContainerMaxSize.width) size.width = ZWTextContainerMaxSize.width;
    if (size.height > ZWTextContainerMaxSize.height) size.height = ZWTextContainerMaxSize.height;
    return size;
}
 ...
 */
static CGColorRef ZWTextGetCGColor(CGColorRef color) {
...
@implementation ZWTextLinePositionSimpleModifier
- (void)modifyLines:(NSArray *)lines fromText:(NSAttributedString *)text inContainer:(ZWTextContainer *)container {
    if (container.verticalForm) {
        for (NSUInteger i = 0, max = lines.count; i < max; i++) {
            ZWTextLine *line = lines[i];
...
- (id)copyWithZone:(NSZone *)zone {
    ZWTextLinePositionSimpleModifier *one = [self.class new];
...
@implementation ZWTextContainer {
    @package
    ...
    ZWTextTruncationType _truncationType;
    NSAttributedString *_truncationToken;
    id<ZWTextLinePositionModifier> _linePositionModifier;
}
...
+ (instancetype)containerWithSize:(CGSize)size insets:(UIEdgeInsets)insets {
    ZWTextContainer *one = [self new];
    one.size = ZWTextClipCGSize(size);
    one.insets = insets;
    return one;
}
...
- (id)copyWithZone:(NSZone *)zone {
    ZWTextContainer *one = [self.class new];
...
- (void)setSize:(CGSize)size {
    Setter(if(!_path) _size = ZWTextClipCGSize(size));
}
...
- (ZWTextTruncationType)truncationType {
    Getter(ZWTextTruncationType type = _truncationType) return type;
}

- (void)setTruncationType:(ZWTextTruncationType)truncationType {
...
- (void)setLinePositionModifier:(id<ZWTextLinePositionModifier>)linePositionModifier {
    Setter(_linePositionModifier = [(NSObject *)linePositionModifier copy]);
}

- (id<ZWTextLinePositionModifier>)linePositionModifier {
    Getter(id<ZWTextLinePositionModifier> m = _linePositionModifier) return m;
}
...
@interface ZWTextLayout ()

@property (nonatomic, readwrite) ZWTextContainer *container;
...
@property (nonatomic, readwrite) ZWTextLine *truncatedLine;
...
@property (nonatomic, assign) ZWRowEdge *lineRowsEdge; ///< top-left origin

@end



@implementation ZWTextLayout
...
+ (ZWTextLayout *)layoutWithContainerSize:(CGSize)size text:(NSAttributedString *)text {
    ZWTextContainer *container = [ZWTextContainer containerWithSize:size];
    return [self layoutWithContainer:container text:text];
}

+ (ZWTextLayout *)layoutWithContainer:(ZWTextContainer *)container text:(NSAttributedString *)text {
    return [self layoutWithContainer:container text:text range:NSMakeRange(0, text.length)];
}

+ (ZWTextLayout *)layoutWithContainer:(ZWTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range {
    ZWTextLayout *layout = NULL;
    ...
    ZWTextLine *truncatedLine = nil;
    ZWRowEdge *lineRowsEdge = NULL;
    ...
    layout = [[ZWTextLayout alloc] _init];
    layout.text = text;
    ...
    if (container.path == nil && container.exclusionPaths.count == 0) {
        ...
        if (needFixLayoutSizeBug) {
            ...
            if (container.isVerticalForm) {
                rect.size.width = ZWTextContainerMaxSize.width;
            } else {
                rect.size.height = ZWTextContainerMaxSize.height;
            }
        }
    ...
    ctFrame = CTFramesetterCreateFrame(ctSetter, ZWCFRangeFromNSRange(range), cgPath, (CFTypeRef)frameAttrs);
    ...
    for (NSUInteger i = 0; i < lineCount; i++) {
        ...
        ZWTextLine *line = [ZWTextLine lineWithCTLine:ctLine position:position vertical:isVerticalForm];
    ...
    if (rowCount > 0) {
        if (maximumNumberOfRows > 0) {
            if (rowCount > maximumNumberOfRows) {
                ...
                do {
                    ZWTextLine *line = lines.lastObject;
                    if (!line) break;
        ...
        ZWTextLine *lastLine = lines.lastObject;
        ...
        lineRowsEdge = calloc(rowCount, sizeof(ZWRowEdge));
        ...
        for (NSUInteger i = 0, max = lines.count; i < max; i++) {
            ...
            if ((NSInteger)line.row != lastRowIdx) {
                if (lastRowIdx >= 0) {
                    lineRowsEdge[lastRowIdx] = (ZWRowEdge) {.head = lastHead, .foot = lastFoot };
                }
                lastRowIdx = line.row;
        ...
        for (NSUInteger i = 1; i < rowCount; i++) {
            ZWRowEdge v0 = lineRowsEdge[i - 1];
            ZWRowEdge v1 = lineRowsEdge[i];
    ...
    visibleRange = ZWNSRangeFromCFRange(CTFrameGetVisibleStringRange(ctFrame));
    if (needTruncation) {
        ZWTextLine *lastLine = lines.lastObject;
        ...
        if (container.truncationType != ZWTextTruncationTypeNone) {
            ...
            if (container.truncationToken) {
                ...
                truncationToken = [[NSAttributedString alloc] initWithString:ZWTextTruncationToken attributes:attrs];
                truncationTokenLine = CTLineCreateWithAttributedString((CFAttributedStringRef)truncationToken);
            }
            if (truncationTokenLine) {
                CTLineTruncationType type = kCTLineTruncationEnd;
                if (container.truncationType == ZWTextTruncationTypeStart) {
                    type = kCTLineTruncationStart;
                } else if (container.truncationType == ZWTextTruncationTypeMiddle) {
                ...
                if (ctLastLineExtend) {
                    ...
                    if (ctTruncatedLine) {
                        truncatedLine = [ZWTextLine lineWithCTLine:ctTruncatedLine position:lastLine.position vertical:isVerticalForm];
    ...
    if (isVerticalForm) {
        NSCharacterSet *rotateCharset = ZWTextVerticalFormRotateCharacterSet();
        NSCharacterSet *rotateMoveCharset = ZWTextVerticalFormRotateAndMoveCharacterSet();
        
        void (^lineBlock)(ZWTextLine *) = ^(ZWTextLine *line){
            ...
            for (NSUInteger r = 0; r < runCount; r++) {
                ...
                ZWTextRunGlyphDrawMode prevMode = ZWTextRunGlyphDrawModeHorizontal;
                ...
                for (NSUInteger g = 0; g < glyphCount; g++) {
                    ...
                    ZWTextRunGlyphDrawMode mode = glyphRotateMove ? ZWTextRunGlyphDrawModeVerticalRotateMove : (glyphRotate ? ZWTextRunGlyphDrawModeVerticalRotate : ZWTextRunGlyphDrawModeHorizontal);
                    if (g == 0) {
                        prevMode = mode;
                    } else if (mode != prevMode) {
                        ZWTextRunGlyphRange *aRange = [ZWTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, g - prevIdx) drawMode:prevMode];
                ...
                if (prevIdx < glyphCount) {
                    ZWTextRunGlyphRange *aRange = [ZWTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, glyphCount - prevIdx) drawMode:prevMode];
                    [runRanges addObject:aRange];
                }
                
            }
        };
        for (ZWTextLine *line in lines) {
            lineBlock(line);
        }
    ...
    if (visibleRange.length > 0) {
        ...
        void (^block)(NSDictionary *attrs, NSRange range, BOOL *stop) = ^(NSDictionary *attrs, NSRange range, BOOL *stop) {
            if (attrs[ZWTextHighlightAttributeName]) layout.containsHighlight = YES;
            if (attrs[ZWTextBlockBorderAttributeName]) layout.needDrawBlockBorder = YES;
            if (attrs[ZWTextBackgroundBorderAttributeName]) layout.needDrawBackgroundBorder = YES;
            if (attrs[ZWTextShadowAttributeName] || attrs[NSShadowAttributeName]) layout.needDrawShadow = YES;
            if (attrs[ZWTextUnderlineAttributeName]) layout.needDrawUnderline = YES;
            if (attrs[ZWTextAttachmentAttributeName]) layout.needDrawAttachment = YES;
            if (attrs[ZWTextInnerShadowAttributeName]) layout.needDrawInnerShadow = YES;
            if (attrs[ZWTextStrikethroughAttributeName]) layout.needDrawStrikethrough = YES;
            if (attrs[ZWTextBorderAttributeName]) layout.needDrawBorder = YES;
        };
    ...
    for (NSUInteger i = 0, max = lines.count; i < max; i++) {
        ...
        if (line.attachments.count > 0) {
            ...
            for (ZWTextAttachment *attachment in line.attachments) {
...
+ (NSArray *)layoutWithContainers:(NSArray *)containers text:(NSAttributedString *)text range:(NSRange)range {
    ...
    for (NSUInteger i = 0, max = containers.count; i < max; i++) {
        ZWTextContainer *container = containers[i];
        ZWTextLayout *layout = [self layoutWithContainer:container text:text range:range];
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    NSData *textData = [ZWTextArchiver archivedDataWithRootObject:_text];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    NSAttributedString *text = [ZWTextUnarchiver unarchiveObjectWithData:textData];
    ZWTextContainer *container = [aDecoder decodeObjectForKey:@"container"];
...
- (NSUInteger)_rowIndexForEdge:(CGFloat)edge {
    ...
    while (lo <= hi) {
        mid = (lo + hi) / 2;
        ZWRowEdge oneEdge = _lineRowsEdge[mid];
        if (isVertical ?
 ...
 */
- (CTRunRef)_runForLine:(ZWTextLine *)line position:(ZWTextPosition *)position {
    if (!line || !position) return NULL;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
    for (NSUInteger i = 0, max = CFArrayGetCount(runs); i < max; i++) {
        CTRunRef run = CFArrayGetValueAtIndex(runs, i);
        CFRange range = CTRunGetStringRange(run);
        if (position.affinity == ZWTextAffinityBackward) {
...
/**
 ...
 next:  right position
 */
- (BOOL)_insideComposedCharacterSequences:(ZWTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
 ...
 */
- (BOOL)_insideEmoji:(ZWTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
    if (!line) return NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
...
/**
 ...
 @return YES if RTL.
 */
- (BOOL)_isRightToLeftInLine:(ZWTextLine *)line atPoint:(CGPoint)point {
    if (!line) return NO;
    // get write direction
    BOOL RTL = NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
 ...
 */
- (ZWTextRange *)_correctedRangeWithEdge:(ZWTextRange *)range {
    NSRange visibleRange = self.visibleRange;
    ZWTextPosition *start = range.start;
    ZWTextPosition *end = range.end;
    
    if (start.offset == visibleRange.location && start.affinity == ZWTextAffinityBackward) {
        start = [ZWTextPosition positionWithOffset:start.offset affinity:ZWTextAffinityForward];
    }
    
    if (end.offset == visibleRange.location + visibleRange.length && start.affinity == ZWTextAffinityForward) {
        end = [ZWTextPosition positionWithOffset:end.offset affinity:ZWTextAffinityBackward];
    }
    
    if (start != range.start || end != range.end) {
        range = [ZWTextRange rangeWithStart:start end:end];
    }
    return range;
}
...
- (NSUInteger)rowIndexForLine:(NSUInteger)line {
    ...
    return ((ZWTextLine *)_lines[line]).row;
}
...
- (NSUInteger)lineIndexForPoint:(CGPoint)point {
    ...
    for (NSUInteger i = lineIdx0; i <= lineIdx1; i++) {
        CGRect bounds = ((ZWTextLine *)_lines[i]).bounds;
...
- (CGFloat)offsetForTextPosition:(NSUInteger)position lineIndex:(NSUInteger)lineIndex {
    ...
    ZWTextLine *line = _lines[lineIndex];
...
- (ZWTextPosition *)closestPositionToPoint:(CGPoint)point {
    BOOL isVertical = _container.verticalForm;
    ...
    if (position <= _visibleRange.location) {
        return [ZWTextPosition positionWithOffset:_visibleRange.location affinity:ZWTextAffinityForward];
    } else if (position >= _visibleRange.location + _visibleRange.length) {
        return [ZWTextPosition positionWithOffset:_visibleRange.location + _visibleRange.length affinity:ZWTextAffinityBackward];
    }
    
    ZWTextAffinity finalAffinity = ZWTextAffinityForward;
    BOOL finalAffinityDetected = NO;
    
    // binding range
    NSRange bindingRange;
    ZWTextBinding *binding = [_text attribute:ZWTextBindingAttributeName atIndex:position longestEffectiveRange:&bindingRange inRange:NSMakeRange(0, _text.length)];
    if (binding && bindingRange.length > 0) {
        NSUInteger headLineIdx = [self lineIndexForPosition:[ZWTextPosition positionWithOffset:bindingRange.location]];
        NSUInteger tailLineIdx = [self lineIndexForPosition:[ZWTextPosition positionWithOffset:bindingRange.location + bindingRange.length affinity:ZWTextAffinityBackward]];
            ...
            if (left != CGFLOAT_MAX && right != CGFLOAT_MAX) {
                if (_container.isVerticalForm) {
                    if (fabs(point.y - left) < fabs(point.y - right)) {
                        ...
                        finalAffinity = ZWTextAffinityForward;
                    } else {
                        position = bindingRange.location + bindingRange.length;
                        finalAffinity = ZWTextAffinityBackward;
                    }
                } else {
    ...
    if (line.range.length == 0) {
        ...
        return [ZWTextPosition positionWithOffset:line.range.location affinity:behind ? ZWTextAffinityBackward:ZWTextAffinityForward];
    }
    ...
    if (line.range.length <= 2) {
        ...
        if (ZWTextIsLinebreakString(str)) { // an empty line ("\r", "\n", "\r\n")
            return [ZWTextPosition positionWithOffset:line.range.location];
        }
    }
    ...
    if (position >= line.range.location + line.range.length - 1) {
        if (position > line.range.location) {
            ...
            if (ZWTextIsLinebreakChar(c1)) {
                position--;
                if (position > line.range.location) {
                    unichar c0 = [_text.string characterAtIndex:position - 1];
                    if (ZWTextIsLinebreakChar(c0)) {
                        position--;
                    }
                }
            }
    ...
    if (position == line.range.location) {
        return [ZWTextPosition positionWithOffset:position];
    }
    if (position == line.range.location + line.range.length) {
        return [ZWTextPosition positionWithOffset:position affinity:ZWTextAffinityBackward];
    }
    ...
    if (!finalAffinityDetected) {
        ...
        if (ofs != CGFLOAT_MAX) {
            ...
            if (position >= line.range.location + line.range.length) {
                finalAffinity = RTL ? ZWTextAffinityForward : ZWTextAffinityBackward;
            } else if (position <= line.range.location) {
                finalAffinity = RTL ? ZWTextAffinityBackward : ZWTextAffinityForward;
            } else {
                finalAffinity = (ofs < (isVertical ? point.y : point.x) && !RTL) ? ZWTextAffinityForward : ZWTextAffinityBackward;
            }
        }
    }
    
    return [ZWTextPosition positionWithOffset:position affinity:finalAffinity];
}

- (ZWTextPosition *)positionForPoint:(CGPoint)point
                         oldPosition:(ZWTextPosition *)oldPosition
                       otherPosition:(ZWTextPosition *)otherPosition {
    if (!oldPosition || !otherPosition) {
        return oldPosition;
    }
    ZWTextPosition *newPos = [self closestPositionToPoint:point];
    ...
    ZWTextLine *line = _lines[lineIndex];
    ZWRowEdge vertical = _lineRowsEdge[line.row];
    if (_container.verticalForm) {
    ...
    if (_container.isVerticalForm) {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            ZWTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionUp offset:1];
            if (range) return range.start;
        } else { // search forward
            ZWTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionDown offset:1];
            if (range) return range.end;
        }
    } else {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            ZWTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionLeft offset:1];
            if (range) return range.start;
        } else { // search forward
            ZWTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionRight offset:1];
            if (range) return range.end;
        }
    }
...
- (ZWTextRange *)textRangeAtPoint:(CGPoint)point {
    ...
    ZWTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    if (_container.verticalForm) {
        ZWTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.y >= point.y && !RTL) ? UITextLayoutDirectionUp:UITextLayoutDirectionDown offset:1];
        return range;
    } else {
        ZWTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.x >= point.x && !RTL) ? UITextLayoutDirectionLeft:UITextLayoutDirectionRight offset:1];
        return range;
    }
}

- (ZWTextRange *)closestTextRangeAtPoint:(CGPoint)point {
    ZWTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    ZWTextRange *range = [self textRangeByExtendingPosition:pos inDirection:direction offset:1];
    return range;
}

- (ZWTextRange *)textRangeByExtendingPosition:(ZWTextPosition *)position {
    ...
    if (position.offset == visibleStart) {
        return [ZWTextRange rangeWithRange:NSMakeRange(position.offset, 0)];
    } else if (position.offset == visibleEnd) {
        return [ZWTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:ZWTextAffinityBackward];
    }
    
    // binding range
    NSRange tRange;
    ZWTextBinding *binding = [_text attribute:ZWTextBindingAttributeName atIndex:position.offset longestEffectiveRange:&tRange inRange:_visibleRange];
    if (binding && tRange.length > 0 && tRange.location < position.offset) {
        return [ZWTextRange rangeWithRange:tRange];
    }
    ...
    if (lineIndex != NSNotFound) {
        ...
        if (emoji || seq) {
            return [ZWTextRange rangeWithRange:NSMakeRange(_prev, _next - _prev)];
        }
    }
    ...
    if (position.offset > visibleStart && position.offset < visibleEnd) {
        ...
        if ((c0 == '\r') && position.offset < visibleEnd) {
            ...
            if (c1 == '\n') {
                return [ZWTextRange rangeWithStart:[ZWTextPosition positionWithOffset:position.offset - 1] end:[ZWTextPosition positionWithOffset:position.offset + 1]];
            }
        }
        if (ZWTextIsLinebreakChar(c0) && position.affinity == ZWTextAffinityBackward) {
            NSString *str = [_text.string substringToIndex:position.offset];
            NSUInteger len = ZWTextLinebreakTailLength(str);
            return [ZWTextRange rangeWithStart:[ZWTextPosition positionWithOffset:position.offset - len] end:[ZWTextPosition positionWithOffset:position.offset]];
        }
    }
    
    return [ZWTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:position.affinity];
}
...
- (ZWTextRange *)textRangeByExtendingPosition:(ZWTextPosition *)position
    ...
    if (!forwardMove && position.offset == visibleStart) {
        return [ZWTextRange rangeWithRange:NSMakeRange(_visibleRange.location, 0)];
    } else if (forwardMove && position.offset == visibleEnd) {
        return [ZWTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:ZWTextAffinityBackward];
    }
    
    // extend from position
    ZWTextRange *fromRange = [self textRangeByExtendingPosition:position];
    if (!fromRange) return nil;
    ZWTextRange *allForward = [ZWTextRange rangeWithStart:fromRange.start end:[ZWTextPosition positionWithOffset:visibleEnd]];
    ZWTextRange *allBackward = [ZWTextRange rangeWithStart:[ZWTextPosition positionWithOffset:visibleStart] end:fromRange.end];
    ...
    if (verticalMove) { // up/down in text layout
        ...
        ZWTextLine *mostLeftLine = nil, *mostRightLine = nil;
        ...
        ZWTextLine *insideLine = _lines[insideIndex];
        NSUInteger pos;
        if (isVerticalForm) {
        ...
        ZWTextPosition *extPos;
        if (afinityEdge) {
            if (pos == insideLine.range.location + insideLine.range.length) {
                NSString *subStr = [_text.string substringWithRange:insideLine.range];
                NSUInteger lineBreakLen = ZWTextLinebreakTailLength(subStr);
                extPos = [ZWTextPosition positionWithOffset:pos - lineBreakLen];
            } else {
                extPos = [ZWTextPosition positionWithOffset:pos];
            }
        } else {
            extPos = [ZWTextPosition positionWithOffset:pos];
        }
        ZWTextRange *ext = [self textRangeByExtendingPosition:extPos];
        if (!ext) return nil;
        if (forwardMove) {
            return [ZWTextRange rangeWithStart:fromRange.start end:ext.end];
        } else {
            return [ZWTextRange rangeWithStart:ext.start end:fromRange.end];
        }
        
    } else { // left/right in text layout
        ZWTextPosition *toPosition = [ZWTextPosition positionWithOffset:position.offset + (forwardMove ? offset : -offset)];
        if (toPosition.offset <= visibleStart) return allBackward;
        else if (toPosition.offset >= visibleEnd) return allForward;
        
        ZWTextRange *toRange = [self textRangeByExtendingPosition:toPosition];
        ...
        return [ZWTextRange rangeWithRange:NSMakeRange(start, end - start)];
    }
}

- (NSUInteger)lineIndexForPosition:(ZWTextPosition *)position {
...
- (CGPoint)linePositionForPosition:(ZWTextPosition *)position {
    NSUInteger lineIndex = [self lineIndexForPosition:position];
...
- (CGRect)caretRectForPosition:(ZWTextPosition *)position {
    NSUInteger lineIndex = [self lineIndexForPosition:position];
...
- (CGRect)firstRectForRange:(ZWTextRange *)range {
    ...
    ZWTextLine *startLine = _lines[startLineIndex];
    ZWTextLine *endLine = _lines[endLineIndex];
    NSMutableArray *lines = [NSMutableArray new];
    for (NSUInteger i = startLineIndex; i <= startLineIndex; i++) {
        ZWTextLine *line = _lines[i];
    ...
    if (_container.verticalForm) {
        if (lines.count == 1) {
            ...
            if (top > bottom) ZW_SWAP(top, bottom);
        ...
        if (lines.count == 1) {
            ...
            if (left > right) ZW_SWAP(left, right);
...
- (CGRect)rectForRange:(ZWTextRange *)range {
    NSArray *rects = [self selectionRectsForRange:range];
    if (rects.count == 0) return CGRectNull;
    CGRect rectUnion = ((ZWTextSelectionRect *)rects.firstObject).rect;
    for (NSUInteger i = 1; i < rects.count; i++) {
        ZWTextSelectionRect *rect = rects[i];
        rectUnion = CGRectUnion(rectUnion, rect.rect);
    }
    return rectUnion;
}

- (NSArray *)selectionRectsForRange:(ZWTextRange *)range {
    range = [self _correctedRangeWithEdge:range];
    ...
    if (startLineIndex > endLineIndex) ZW_SWAP(startLineIndex, endLineIndex);
    ZWTextLine *startLine = _lines[startLineIndex];
    ZWTextLine *endLine = _lines[endLineIndex];
    ...
    ZWTextSelectionRect *start = [ZWTextSelectionRect new];
    if (isVertical) {
    ...
    ZWTextSelectionRect *end = [ZWTextSelectionRect new];
    if (isVertical) {
    ...
    if (startLine.row == endLine.row) { // same row
        if (offsetStart > offsetEnd) ZW_SWAP(offsetStart, offsetEnd);
        ZWTextSelectionRect *rect = [ZWTextSelectionRect new];
        if (isVertical) {
        ...
        ZWTextSelectionRect *topRect = [ZWTextSelectionRect new];
        ...
        ZWTextSelectionRect *bottomRect = [ZWTextSelectionRect new];
...
- (NSArray *)selectionRectsWithoutStartAndEndForRange:(ZWTextRange *)range {
...
- (NSArray *)selectionRectsWithOnlyStartAndEndForRange:(ZWTextRange *)range {
    NSMutableArray *rects = [self selectionRectsForRange:range].mutableCopy;
    for (NSInteger i = 0, max = rects.count; i < max; i++) {
        ZWTextSelectionRect *rect = rects[i];
        if (!rect.containsStart && !rect.containsEnd) {
...
typedef NS_OPTIONS(NSUInteger, ZWTextDecorationType) {
    ZWTextDecorationTypeUnderline     = 1 << 0,
    ZWTextDecorationTypeStrikethrough = 1 << 1,
};

typedef NS_OPTIONS(NSUInteger, ZWTextBorderType) {
    ZWTextBorderTypeBackgound = 1 << 0,
    ZWTextBorderTypeNormal    = 1 << 1,
};

static CGRect ZWTextMergeRectInSameLine(CGRect rect1, CGRect rect2, BOOL isVertical) {
    if (isVertical) {
...
static void ZWTextGetRunsMaxMetric(CFArrayRef runs, CGFloat *xHeight, CGFloat *underlinePosition, CGFloat *lineThickness) {
...
static void ZWTextDrawRun(ZWTextLine *line, CTRunRef run, CGContextRef context, CGSize size, BOOL isVertical, NSArray *runRanges, CGFloat verticalOffset) {
    CGAffineTransform runTextMatrix = CTRunGetTextMatrix(run);
    BOOL runTextMatrixIsID = CGAffineTransformIsIdentity(runTextMatrix);
    
    CFDictionaryRef runAttrs = CTRunGetAttributes(run);
    NSValue *glyphTransformValue = CFDictionaryGetValue(runAttrs, (__bridge const void *)(ZWTextGlyphTransformAttributeName));
    if (!isVertical && !glyphTransformValue) { // draw run
        ...
        fillColor = ZWTextGetCGColor(fillColor);
        ...
        CGContextSaveGState(context); {
            ...
            if (isVertical) {
                ...
                for (ZWTextRunGlyphRange *oneRange in runRanges) {
                    NSRange range = oneRange.glyphRangeInRun;
                    NSUInteger rangeMax = range.location + range.length;
                    ZWTextRunGlyphDrawMode mode = oneRange.drawMode;
                    ...
                    for (NSUInteger g = range.location; g < rangeMax; g++) {
                        CGContextSaveGState(context); {
                            ...
                            if (mode) { // CJK glyph, need rotated
                                ...
                                if (mode == ZWTextRunGlyphDrawModeVerticalRotateMove) {
                                    x += w;
                                    y += w;
                                }
...
static void ZWTextSetLinePatternInContext(ZWTextLineStyle style, CGFloat width, CGFloat phase, CGContextRef context){
    ...
    } else if (pattern == ZWTextLineStylePatternCircleDot) {
...
static void ZWTextDrawBorderRects(CGContextRef context, CGSize size, ZWTextBorder *border, NSArray *rects, BOOL isVertical) {
    if (rects.count == 0) return;
    
    ZWTextShadow *shadow = border.shadow;
    if (shadow.color) {
        CGContextSaveGState(context);
    ...
    if (border.strokeColor && border.lineStyle > 0 && border.strokeWidth > 0) {
        ...
        ZWTextSetLinePatternInContext(border.lineStyle, border.strokeWidth, 0, context);
        CGFloat inset = -border.strokeWidth * 0.5;
        if ((border.lineStyle & 0xFF) == ZWTextLineStyleThick) {
            inset *= 2;
        ...
        //------------------------- second line ------------------------------//
        if ((border.lineStyle & 0xFF) == ZWTextLineStyleDouble) {
            CGContextSaveGState(context);
...
static void ZWTextDrawLineStyle(CGContextRef context, CGFloat length, CGFloat lineWidth, ZWTextLineStyle style, CGPoint position, CGColorRef color, BOOL isVertical) {
    ...
    CGContextSaveGState(context); {
        if (isVertical) {
            ...
            w = (styleBase == ZWTextLineStyleThick ? lineWidth * 2 : lineWidth);
            ...
            ZWTextSetLinePatternInContext(style, lineWidth, position.y, context);
            CGContextSetLineWidth(context, w);
            if (styleBase == ZWTextLineStyleSingle) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == ZWTextLineStyleThick) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == ZWTextLineStyleDouble) {
            ...
            CGContextSetStrokeColorWithColor(context, color);
            ZWTextSetLinePatternInContext(style, lineWidth, position.x, context);
            CGContextSetLineWidth(context, w);
            if (styleBase == ZWTextLineStyleSingle) {
...
static void ZWTextDrawText(ZWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                ZWTextDrawRun(line, run, context, size, isVertical, lineRunRanges[r], verticalOffset);
            }
...
static void ZWTextDrawBlockBorder(ZWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            ZWTextBorder *border = attrs[ZWTextBlockBorderAttributeName];
            if (!border) continue;
            
            NSUInteger lineStartIndex = line.index;
            while (lineStartIndex > 0) {
                if (((ZWTextLine *)lines[lineStartIndex - 1]).row == line.row) lineStartIndex--;
                else break;
            }
            
            CGRect unionRect = CGRectZero;
            NSUInteger lineStartRow = ((ZWTextLine *)lines[lineStartIndex]).row;
            NSUInteger lineContinueIndex = lineStartIndex;
            NSUInteger lineContinueRow = lineStartRow;
            do {
                ZWTextLine *one = lines[lineContinueIndex];
                ...
                ZWTextLine *next = lines[lineContinueIndex + 1];
                if (next.row != lineContinueRow) {
                    ZWTextBorder *nextBorder = [layout.text attribute:ZWTextBlockBorderAttributeName atIndex:next.range.location];
            ...
            ZWTextDrawBorderRects(context, size, border, @[[NSValue valueWithCGRect:unionRect]], isVertical);
...
static void ZWTextDrawBorder(ZWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, ZWTextBorderType type, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    NSArray *lines = layout.lines;
    NSString *borderKey = (type == ZWTextBorderTypeNormal ? ZWTextBorderAttributeName : ZWTextBackgroundBorderAttributeName);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            ZWTextBorder *border = attrs[borderKey];
            if (!border) continue;
            ...
            for (NSInteger ll = l; ll < lMax; ll++) {
                ...
                for (NSInteger rr = (ll == l) ? r : 0, rrMax = CFArrayGetCount(iRuns); rr < rrMax; rr++) {
                    ...
                    ZWTextBorder *iBorder = iAttrs[borderKey];
                    ...
                    if (isVertical) {
                        ZW_SWAP(iRunPosition.x, iRunPosition.y);
            ...
            for (NSInteger re = 0, reMax = runRects.count; re < reMax; re++) {
                ...
                if (isVertical) {
                    if (fabs(rect.origin.x - curRect.origin.x) < 1) {
                        curRect = ZWTextMergeRectInSameLine(rect, curRect, isVertical);
                    } else {
            ...
            ZWTextDrawBorderRects(context, size, border, drawRects, isVertical);
...
static void ZWTextDrawDecoration(ZWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, ZWTextDecorationType type, BOOL (^cancel)(void)) {
    NSArray *lines = layout.lines;
    ...
    for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            ZWTextDecoration *underline = attrs[ZWTextUnderlineAttributeName];
            ZWTextDecoration *strikethrough = attrs[ZWTextStrikethroughAttributeName];
            
            BOOL needDrawUnderline = NO, needDrawStrikethrough = NO;
            if ((type & ZWTextDecorationTypeUnderline) && underline.style > 0) {
                needDrawUnderline = YES;
            }
            if ((type & ZWTextDecorationTypeStrikethrough) && strikethrough.style > 0) {
            ...
            if (ZWTextIsLinebreakString(runStr)) continue; // may need more checks...
            
            CGFloat xHeight, underlinePosition, lineThickness;
            ZWTextGetRunsMaxMetric(runs, &xHeight, &underlinePosition, &lineThickness);
                ...
                while (shadow) {
                    ...
                    CGContextSaveGState(context); {
                        ...
                        CGContextSaveGState(context); {
                            ...
                            ZWTextDrawLineStyle(context, length, thickness, underline.style, underlineStart, color, isVertical);
                        } CGContextRestoreGState(context);
                    } CGContextRestoreGState(context);
                    shadow = shadow.subShadow;
                }
            ...
            if (needDrawStrikethrough) {
                ...
                ZWTextDrawLineStyle(context, length, thickness, strikethrough.style, strikethroughStart, color, isVertical);
            }
        }
    }
    CGContextRestoreGState(context);
...
static void ZWTextDrawAttachment(ZWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, UIView *targetView, CALayer *targetLayer, BOOL (^cancel)(void)) {
    
    BOOL isVertical = layout.container.verticalForm;
    CGFloat verticalOffset = isVertical ? (size.width - layout.container.size.width) : 0;
    
    for (NSUInteger i = 0, max = layout.attachments.count; i < max; i++) {
        ZWTextAttachment *a = layout.attachments[i];
        ...
        rect = ZWCGRectFitWithContentMode(rect, asize, a.contentMode);
        rect = CGRectPixelRound(rect);
        rect = CGRectStandardize(rect);
...
static void ZWTextDrawShadow(ZWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    ...
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                NSDictionary *attrs = (id)CTRunGetAttributes(run);
                ZWTextShadow *shadow = attrs[ZWTextShadowAttributeName];
                ZWTextShadow *nsShadow = [ZWTextShadow shadowWithNSShadow:attrs[NSShadowAttributeName]]; // NSShadow compatible
                if (nsShadow) {
...
static void ZWTextDrawInnerShadow(ZWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextTranslateCTM(context, 0, size.height);
    CGContextScaleCTM(context, 1, -1);
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            CGContextSetTextPosition(context, linePosX, linePosY);
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            ZWTextShadow *shadow = attrs[ZWTextInnerShadowAttributeName];
            while (shadow) {
                if (!shadow.color) {
                    shadow = shadow.subShadow;
                    continue;
...
static void ZWTextDrawDebug(ZWTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, ZWTextDebugOption *op) {
    UIGraphicsPushContext(context);
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextSetLineWidth(context, 1.0 / ZWScreenScale());
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        if (op.CTRunFillColor || op.CTRunBorderColor || op.CTRunNumberColor || op.CGGlyphFillColor || op.CGGlyphBorderColor) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                if (isVertical) {
                    ZW_SWAP(runPosition.x, runPosition.y);
...
- (void)drawInContext:(CGContextRef)context
                ...
                point:(CGPoint)point
                 view:(UIView *)view
                layer:(CALayer *)layer
                debug:(ZWTextDebugOption *)debug
               cancel:(BOOL (^)(void))cancel{
    @autoreleasepool {
        if (self.needDrawBlockBorder && context) {
            if (cancel && cancel()) return;
            ZWTextDrawBlockBorder(self, context, size, point, cancel);
        }
        if (self.needDrawBackgroundBorder && context) {
            if (cancel && cancel()) return;
            ZWTextDrawBorder(self, context, size, point, ZWTextBorderTypeBackgound, cancel);
        }
        if (self.needDrawShadow && context) {
            if (cancel && cancel()) return;
            ZWTextDrawShadow(self, context, size, point, cancel);
        }
        if (self.needDrawUnderline && context) {
            if (cancel && cancel()) return;
            ZWTextDrawDecoration(self, context, size, point, ZWTextDecorationTypeUnderline, cancel);
        }
        if (self.needDrawText && context) {
            if (cancel && cancel()) return;
            ZWTextDrawText(self, context, size, point, cancel);
        }
        if (self.needDrawAttachment && (context || view || layer)) {
            if (cancel && cancel()) return;
            ZWTextDrawAttachment(self, context, size, point, view, layer, cancel);
        }
        if (self.needDrawInnerShadow && context) {
            if (cancel && cancel()) return;
            ZWTextDrawInnerShadow(self, context, size, point, cancel);
        }
        if (self.needDrawStrikethrough && context) {
            if (cancel && cancel()) return;
            ZWTextDrawDecoration(self, context, size, point, ZWTextDecorationTypeStrikethrough, cancel);
        }
        if (self.needDrawBorder && context) {
            if (cancel && cancel()) return;
            ZWTextDrawBorder(self, context, size, point, ZWTextBorderTypeNormal, cancel);
        }
        if (debug.needDrawDebug && context) {
            if (cancel && cancel()) return;
            ZWTextDrawDebug(self, context, size, point, debug);
        }
    }
}
...
- (void)removeAttachmentFromViewAndLayer {
    ZWAssertMainThread();
    for (ZWTextAttachment *a in self.attachments) {
