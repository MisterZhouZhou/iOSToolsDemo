...
@implementation ZWTextEffectWindow

+ (instancetype)sharedWindow {
    static ZWTextEffectWindow *one = nil;
    if (one == nil) {
        // iOS 9 compatible
...
- (ZWTextDirection)_keyboardDirection {
    CGRect keyboardFrame = [ZWTextKeyboardManager defaultManager].keyboardFrame;
    keyboardFrame = [[ZWTextKeyboardManager defaultManager] convertRect:keyboardFrame toView:self];
    if (CGRectIsNull(keyboardFrame) || CGRectIsEmpty(keyboardFrame)) return ZWTextDirectionNone;
    
    if (CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return ZWTextDirectionTop;
    ...
    if (CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame) &&
        ...
        return ZWTextDirectionRight;
    
    if (CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame) &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return ZWTextDirectionBottom;
    
    if (CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame))
        return ZWTextDirectionLeft;
    
    return ZWTextDirectionNone;
}
...
- (CGPoint)_correctedCenter:(CGPoint)center forMagnifier:(ZWTextMagnifier *)mag rotation:(CGFloat)rotation {
    ...
    if (degree <= 1 || degree >= 7) { //top
        if (mag.type == ZWTextMagnifierTypeCaret) {
            if (center.y < caretExt)
                center.y = caretExt;
        } else if (mag.type == ZWTextMagnifierTypeRanged) {
 ...
 */
- (CGFloat)_updateMagnifier:(ZWTextMagnifier *)mag {
    UIApplication *app = [UIApplication sharedExtensionApplication];
    if (!app) return 0;
    ...
    CGAffineTransform trans = ZWCGAffineTransformGetFromViews(hostView, self);
    ...
    for (UIWindow *window in windows) {
        ...
        CGContextConcatCTM(context, ZWCGAffineTransformGetFromViews(window, self));
...
- (void)showMagnifier:(ZWTextMagnifier *)mag {
    ...
    mag.transform = trans;
    mag.center = center;
    if (mag.type == ZWTextMagnifierTypeRanged) {
        mag.alpha = 0;
    }
    NSTimeInterval time = mag.type == ZWTextMagnifierTypeCaret ? 0.08 : 0.1;
...
- (void)hideMagnifier:(ZWTextMagnifier *)mag {
    ...
    CGFloat rotation = [self _updateMagnifier:mag];
    CGPoint center = [self convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
    NSTimeInterval time = mag.type == ZWTextMagnifierTypeCaret ? 0.20 : 0.15;
    [UIView animateWithDuration:time delay:0 options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionBeginFromCurrentState animations:^{
...
- (void)_updateSelectionGrabberDot:(ZWSelectionGrabberDot *)dot selection:(ZWTextSelectionView *)selection{
