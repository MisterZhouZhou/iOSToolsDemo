...
#import "ZWLabel.h"
#import "ZWAsyncLayer.h"
#import "ZWWeakProxy.h"
#import "ZWCGUtilities.h"
#import "NSAttributedString+ZWText.h"

#if __has_include("ZWDispatchQueuePool.h")
#import "ZWDispatchQueuePool.h"
#endif

#ifdef ZWDispatchQueuePool_h
static dispatch_queue_t ZWLabelGetReleaseQueue() {
    return ZWDispatchQueueGetForQOS(NSQualityOfServiceUtility);
}
#else
static dispatch_queue_t ZWLabelGetReleaseQueue() {
...
@interface ZWLabel() <ZWTextDebugTarget, ZWAsyncLayerDelegate> {
    NSMutableAttributedString *_innerText; ///< nonnull
    ZWTextLayout *_innerLayout;
    ZWTextContainer *_innerContainer; ///< nonnull
    ...
    ZWTextLayout *_shrinkInnerLayout;
    ZWTextLayout *_shrinkHighlightLayout;
...
- (void)_updateLayout {
    _innerLayout = [ZWTextLayout layoutWithContainer:_innerContainer text:_innerText];
    _shrinkInnerLayout = [ZWLabel _shrinkLayoutWithLayout:_innerLayout];
}
...
- (void)_clearInnerLayout {
    ...
    ZWTextLayout *layout = _innerLayout;
    _innerLayout = nil;
    _shrinkInnerLayout = nil;
    dispatch_async(ZWLabelGetReleaseQueue(), ^{
...
- (ZWTextLayout *)_highlightLayout {
    return _shrinkHighlightLayout ? _shrinkHighlightLayout : _highlightLayout;
}

+ (ZWTextLayout *)_shrinkLayoutWithLayout:(ZWTextLayout *)layout {
    if (layout.text.length && layout.lines.count == 0) {
        ZWTextContainer *container = layout.container.copy;
        container.maximumNumberOfRows = 1;
        CGSize containerSize = container.size;
        if (!container.verticalForm) {
            containerSize.height = ZWTextContainerMaxSize.height;
        } else {
            containerSize.width = ZWTextContainerMaxSize.width;
        }
        container.size = containerSize;
        return [ZWTextLayout layoutWithContainer:container text:layout.text];
    } else {
        return nil;
    }
}
...
- (void)_trackDidLongPress {
    ...
    if (_state.hasLongPressAction && _textLongPressAction) {
        ...
        ZWTextRange *textRange = [self._innerLayout textRangeAtPoint:point];
    ...
    if (_highlight) {
        ZWTextAction longPressAction = _highlight.longPressAction ? _highlight.longPressAction : _highlightLongPressAction;
        if (longPressAction) {
            ZWTextPosition *start = [ZWTextPosition positionWithOffset:_highlightRange.location];
            ZWTextPosition *end = [ZWTextPosition positionWithOffset:_highlightRange.location + _highlightRange.length affinity:ZWTextAffinityBackward];
            ZWTextRange *range = [ZWTextRange rangeWithStart:start end:end];
...
- (ZWTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    ...
    ZWTextHighlight *highlight = [_innerText attribute:ZWTextHighlightAttributeName
                                               atIndex:startIndex
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        _shrinkHighlightLayout = [ZWLabel _shrinkLayoutWithLayout:_highlightLayout];
...
- (void)_updateOuterLineBreakMode {
    if (_innerContainer.truncationType) {
        switch (_innerContainer.truncationType) {
            case ZWTextTruncationTypeStart: {
                _lineBreakMode = NSLineBreakByTruncatingHead;
            } break;
            case ZWTextTruncationTypeEnd: {
                _lineBreakMode = NSLineBreakByTruncatingTail;
            } break;
            case ZWTextTruncationTypeMiddle: {
...
- (void)_initLabel {
    ((ZWAsyncLayer *)self.layer).displaysAsynchronously = NO;
    ...
    _debugOption = [ZWTextDebugOption sharedDebugOption];
    [ZWTextDebugOption addDebugTarget:self];
    
    _font = [self _defaultFont];
    _textColor = [UIColor blackColor];
    _textVerticalAlignment = ZWTextVerticalAlignmentCenter;
    _numberOfLines = 1;
    _textAlignment = NSTextAlignmentNatural;
    _lineBreakMode = NSLineBreakByTruncatingTail;
    _innerText = [NSMutableAttributedString new];
    _innerContainer = [ZWTextContainer new];
    _innerContainer.truncationType = ZWTextTruncationTypeEnd;
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    if ((!_verticalForm && size.width == self.bounds.size.width) ||
        ...
        ZWTextLayout *layout = self._innerLayout;
        BOOL contains = NO;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    [self _initLabel];
    ZWTextContainer *innerContainer = [aDecoder decodeObjectForKey:@"innerContainer"];
    if (innerContainer) {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_state.touchMoved && _highlight) {
            ZWTextHighlight *highlight = [self _getHighlightAtPoint:point range:NULL];
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            if (!_state.touchMoved || [self _getHighlightAtPoint:point range:NULL] == _highlight) {
                ZWTextAction tapAction = _highlight.tapAction ? _highlight.tapAction : _highlightTapAction;
                if (tapAction) {
                    ZWTextPosition *start = [ZWTextPosition positionWithOffset:_highlightRange.location];
                    ZWTextPosition *end = [ZWTextPosition positionWithOffset:_highlightRange.location + _highlightRange.length affinity:ZWTextAffinityBackward];
                    ZWTextRange *range = [ZWTextRange rangeWithStart:start end:end];
...
- (void)setLineBreakMode:(NSLineBreakMode)lineBreakMode {
    ...
    switch (lineBreakMode) {
        ...
        case NSLineBreakByCharWrapping:
        case NSLineBreakByClipping: {
            _innerContainer.truncationType = ZWTextTruncationTypeNone;
            _innerText.lineBreakMode = lineBreakMode;
        } break;
        case NSLineBreakByTruncatingHead:{
            _innerContainer.truncationType = ZWTextTruncationTypeStart;
            _innerText.lineBreakMode = NSLineBreakByWordWrapping;
        } break;
        case NSLineBreakByTruncatingTail:{
            _innerContainer.truncationType = ZWTextTruncationTypeEnd;
        ...
        case NSLineBreakByTruncatingMiddle: {
            _innerContainer.truncationType = ZWTextTruncationTypeMiddle;
            _innerText.lineBreakMode = NSLineBreakByWordWrapping;
        } break;
        default: break;
    }
...
- (void)setTextLayout:(ZWTextLayout *)textLayout {
...
- (void)setDisplaysAsynchronously:(BOOL)displaysAsynchronously {
    ...
    ((ZWAsyncLayer *)self.layer).displaysAsynchronously = displaysAsynchronously;
}
...
- (CGSize)intrinsicContentSize {
    if (_preferredMaxLayoutWidth == 0) {
        ZWTextContainer *container = [_innerContainer copy];
        container.size = ZWTextContainerMaxSize;
...
- (ZWAsyncLayerDisplayTask *)newAsyncDisplayTask {
    
    // capture current context
    BOOL contentsNeedFade = _state.contentsNeedFade;
    NSAttributedString *text = _innerText;
    ZWTextContainer *container = _innerContainer;
    ZWTextVerticalAlignment verticalAlignment = _textVerticalAlignment;
    ZWTextDebugOption *debug = _debugOption;
    ...
    __block ZWTextLayout *layout = (_state.showingHighlight && _highlightLayout) ? self._highlightLayout : self._innerLayout;
    __block ZWTextLayout *shrinkLayout = nil;
    ...
    // create display task
    ZWAsyncLayerDisplayTask *task = [ZWAsyncLayerDisplayTask new];
    ...
    task.display = ^(CGContextRef context, CGSize size, BOOL (^isCancelled)(void)) {
        ...
        ZWTextLayout *drawLayout = layout;
        if (layoutNeedUpdate) {
            layout = [ZWTextLayout layoutWithContainer:container text:text];
            shrinkLayout = [ZWLabel _shrinkLayoutWithLayout:layout];
            if (isCancelled()) return;
            layoutUpdated = YES;
        ...
        if (verticalAlignment == ZWTextVerticalAlignmentCenter) {
        ...
        } else if (verticalAlignment == ZWTextVerticalAlignmentBottom) {
            if (drawLayout.container.isVerticalForm) {
    ...
    task.didDisplay = ^(CALayer *layer, BOOL finished) {
        ...
        if (!finished) {
            ...
            for (ZWTextAttachment *a in drawLayout.attachments) {
        ...
        __strong ZWLabel *view = (ZWLabel *)layer.delegate;
        if (!view) return;
...
@interface ZWLabel(IBInspectableProperties)
@end

@implementation ZWLabel (IBInspectableProperties)
