...
/**
 ...
 @return YES or NO.
 */
static inline BOOL ZWTextIsLinebreakChar(unichar c) {
    switch (c) {
        case 0x000D:
        case 0x2028:
        case 0x000A:
...
/**
 ...
 @return YES or NO.
 */
static inline BOOL ZWTextIsLinebreakString(NSString * _Nullable str) {
    if (str.length > 2 || str.length == 0) return NO;
    if (str.length == 1) {
        unichar c = [str characterAtIndex:0];
        return ZWTextIsLinebreakChar(c);
    } else {
 ...
 */
static inline NSUInteger ZWTextLinebreakTailLength(NSString * _Nullable str) {
    if (str.length >= 2) {
        unichar c2 = [str characterAtIndex:str.length - 1];
        if (ZWTextIsLinebreakChar(c2)) {
        ...
        return ZWTextIsLinebreakChar([str characterAtIndex:0]) ? 1 : 0;
    } else {
        return 0;
    }
}
 ...
 */
static inline CGFloat ZWEmojiGetAscentWithFontSize(CGFloat fontSize) {
    if (fontSize < 16) {
 ...
 */
static inline CGFloat ZWEmojiGetDescentWithFontSize(CGFloat fontSize) {
    if (fontSize < 16) {
 ...
 */
static inline CGRect ZWEmojiGetGlyphBoundingRectWithFontSize(CGFloat fontSize) {
    CGRect rect;
    rect.origin.x = 0.75;
    rect.size.width = rect.size.height = ZWEmojiGetAscentWithFontSize(fontSize);
    if (fontSize < 16) {
 ...
 */
NSCharacterSet *ZWTextVerticalFormRotateCharacterSet(void);

/**
 Get the character set which should rotate and move in vertical form.
 @return The shared character set.
 */
NSCharacterSet *ZWTextVerticalFormRotateAndMoveCharacterSet(void);
