...
//  OCToolsDemo
//
//  Created by chengfei on 2017/12/21.
//  Copyright © 2017年 chengfei. All rights reserved.
//

#import <UIKit/UIKit.h>

@class ZWMemoryCache, ZWDiskCache;

NS_ASSUME_NONNULL_BEGIN

/// Image cache type, 这里使用位移枚举可以兼容多个选项的枚举
typedef NS_OPTIONS(NSUInteger, ZWImageCacheType) {
    /// No value.
    ZWImageCacheTypeNone   = 0,
    
    /// Get/store image with memory cache.
    ZWImageCacheTypeMemory = 1 << 0,
    
    /// Get/store image with disk cache.
    ZWImageCacheTypeDisk   = 1 << 1,
    
    /// Get/store image with both memory cache and disk cache.
    ZWImageCacheTypeAll    = ZWImageCacheTypeMemory | ZWImageCacheTypeDisk,
};
...
@property (strong, readonly) ZWMemoryCache *memoryCache;

/** The underlying disk cache. see `YYDiskCache` for more information.*/
@property (strong, readonly) ZWDiskCache *diskCache;
...
- (void)setImage:(nullable UIImage *)image
       imageData:(nullable NSData *)imageData
          forKey:(NSString *)key
        withType:(ZWImageCacheType)type;
 ...
 */
- (void)removeImageForKey:(NSString *)key withType:(ZWImageCacheType)type;
...
/**
 ...
 @return Whether the image is in cache.
 */
- (BOOL)containsImageForKey:(NSString *)key withType:(ZWImageCacheType)type;
...
/**
 ...
 @return The image associated with key, or nil if no image is associated with key.
 */
- (nullable UIImage *)getImageForKey:(NSString *)key withType:(ZWImageCacheType)type;
...
- (void)getImageForKey:(NSString *)key
              withType:(ZWImageCacheType)type
             withBlock:(void(^)(UIImage * _Nullable image, ZWImageCacheType type))block;
