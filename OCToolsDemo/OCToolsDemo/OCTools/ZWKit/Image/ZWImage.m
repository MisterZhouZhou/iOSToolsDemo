...
+ (ZWImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (ZWImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (ZWImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        ZWImageDecoder *decoder = [ZWImageDecoder decoderWithData:data scale:scale];
        ZWImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
