...
#define ZW_FOUR_CC(c1,c2,c3,c4) ((uint32_t)(((c4) << 24) | ((c3) << 16) | ((c2) << 8) | (c1)))
#define ZW_TWO_CC(c1,c2) ((uint16_t)(((c2) << 8) | (c1)))

static inline uint16_t zw_swap_endian_uint16(uint16_t value) {
    return
    (uint16_t) ((value & 0x00FF) << 8) |
    (uint16_t) ((value & 0xFF00) >> 8) ;
}

static inline uint32_t zw_swap_endian_uint32(uint32_t value) {
    return
...
typedef enum {
    ZW_PNG_ALPHA_TYPE_PALEETE = 1 << 0,
    ZW_PNG_ALPHA_TYPE_COLOR = 1 << 1,
    ZW_PNG_ALPHA_TYPE_ALPHA = 1 << 2,
} zw_png_alpha_type;

typedef enum {
    ZW_PNG_DISPOSE_OP_NONE = 0,
    ZW_PNG_DISPOSE_OP_BACKGROUND = 1,
    ZW_PNG_DISPOSE_OP_PREVIOUS = 2,
} zw_png_dispose_op;

typedef enum {
    ZW_PNG_BLEND_OP_SOURCE = 0,
    ZW_PNG_BLEND_OP_OVER = 1,
} zw_png_blend_op;
...
typedef struct {
    ...
    zw_png_chunk_fcTL frame_control;
} zw_png_frame_info;

typedef struct {
    zw_png_chunk_IHDR header;   ///< png header
    zw_png_chunk_info *chunks;      ///< chunks
    uint32_t chunk_num;          ///< count of chunks
    
    zw_png_frame_info *apng_frames; ///< frame info, NULL if not apng
...
static void zw_png_chunk_IHDR_read(zw_png_chunk_IHDR *IHDR, const uint8_t *data) {
    IHDR->width = zw_swap_endian_uint32(*((uint32_t *)(data)));
    IHDR->height = zw_swap_endian_uint32(*((uint32_t *)(data + 4)));
...
static void zw_png_chunk_IHDR_write(zw_png_chunk_IHDR *IHDR, uint8_t *data) {
    *((uint32_t *)(data)) = zw_swap_endian_uint32(IHDR->width);
    *((uint32_t *)(data + 4)) = zw_swap_endian_uint32(IHDR->height);
...
static void zw_png_chunk_fcTL_read(zw_png_chunk_fcTL *fcTL, const uint8_t *data) {
    fcTL->sequence_number = zw_swap_endian_uint32(*((uint32_t *)(data)));
    fcTL->width = zw_swap_endian_uint32(*((uint32_t *)(data + 4)));
    fcTL->height = zw_swap_endian_uint32(*((uint32_t *)(data + 8)));
    fcTL->x_offset = zw_swap_endian_uint32(*((uint32_t *)(data + 12)));
    fcTL->y_offset = zw_swap_endian_uint32(*((uint32_t *)(data + 16)));
    fcTL->delay_num = zw_swap_endian_uint16(*((uint16_t *)(data + 20)));
    fcTL->delay_den = zw_swap_endian_uint16(*((uint16_t *)(data + 22)));
    fcTL->dispose_op = data[24];
    fcTL->blend_op = data[25];
}

static void zw_png_chunk_fcTL_write(zw_png_chunk_fcTL *fcTL, uint8_t *data) {
    *((uint32_t *)(data)) = zw_swap_endian_uint32(fcTL->sequence_number);
    *((uint32_t *)(data + 4)) = zw_swap_endian_uint32(fcTL->width);
    *((uint32_t *)(data + 8)) = zw_swap_endian_uint32(fcTL->height);
    *((uint32_t *)(data + 12)) = zw_swap_endian_uint32(fcTL->x_offset);
    *((uint32_t *)(data + 16)) = zw_swap_endian_uint32(fcTL->y_offset);
    *((uint16_t *)(data + 20)) = zw_swap_endian_uint16(fcTL->delay_num);
    *((uint16_t *)(data + 22)) = zw_swap_endian_uint16(fcTL->delay_den);
    data[24] = fcTL->dispose_op;
    data[25] = fcTL->blend_op;
}

// convert double value to fraction
static void zw_png_delay_to_fraction(double duration, uint16_t *num, uint16_t *den) {
    if (duration >= 0xFF) {
        *num = 0xFF;
        *den = 1;
...
static double zw_png_delay_to_seconds(uint16_t num, uint16_t den) {
    if (den == 0) {
        return num / 100.0;
    } else {
...
static bool zw_png_validate_animation_chunk_order(zw_png_chunk_info *chunks,  /* input */
    ...
    if (chunks->fourcc != ZW_FOUR_CC('I', 'H', 'D', 'R')) return false;
    if ((chunks + chunk_num - 1)->fourcc != ZW_FOUR_CC('I', 'E', 'N', 'D')) return false;
    ...
    for (uint32_t i = 0; i < chunk_num; i++) {
        zw_png_chunk_info *chunk = chunks + i;
        switch (chunk->fourcc) {
            case ZW_FOUR_CC('I', 'H', 'D', 'R'): {  // png header
                if (i != 0) return false;
                if (IHDR_num > 0) return false;
                IHDR_num++;
            } break;
            case ZW_FOUR_CC('I', 'D', 'A', 'T'): {  // png data
                if (prev_fourcc != ZW_FOUR_CC('I', 'D', 'A', 'T')) {
                    if (IDAT_num == 0)
                        first_IDAT = i;
                    else
                        return false;
            ...
            case ZW_FOUR_CC('a', 'c', 'T', 'L'): {  // apng control
                if (acTL_num > 0) return false;
                acTL_num++;
            } break;
            case ZW_FOUR_CC('f', 'c', 'T', 'L'): {  // apng frame control
                if (i + 1 == chunk_num) return false;
                if ((chunk + 1)->fourcc != ZW_FOUR_CC('f', 'd', 'A', 'T') &&
                    (chunk + 1)->fourcc != ZW_FOUR_CC('I', 'D', 'A', 'T')) {
                    return false;
                }
                if (fcTL_num == 0) {
                    if ((chunk + 1)->fourcc == ZW_FOUR_CC('I', 'D', 'A', 'T')) {
                        first_frame_cover = true;
                    }
                }
                fcTL_num++;
            } break;
            case ZW_FOUR_CC('f', 'd', 'A', 'T'): {  // apng data
                if (prev_fourcc != ZW_FOUR_CC('f', 'd', 'A', 'T') && prev_fourcc != ZW_FOUR_CC('f', 'c', 'T', 'L')) {
                    return false;
                }
            } break;
        }
...
static void zw_png_info_release(zw_png_info *info) {
    if (info) {
 ...
 */
static zw_png_info *zw_png_info_create(const uint8_t *data, uint32_t length) {
    if (length < 32) return NULL;
    if (*((uint32_t *)data) != ZW_FOUR_CC(0x89, 0x50, 0x4E, 0x47)) return NULL;
    if (*((uint32_t *)(data + 4)) != ZW_FOUR_CC(0x0D, 0x0A, 0x1A, 0x0A)) return NULL;
    
    uint32_t chunk_realloc_num = 16;
    zw_png_chunk_info *chunks = malloc(sizeof(zw_png_chunk_info) * chunk_realloc_num);
    ...
    do {
        if (chunk_num >= chunk_capacity) {
            zw_png_chunk_info *new_chunks = realloc(chunks, sizeof(zw_png_chunk_info) * (chunk_capacity + chunk_realloc_num));
            if (!new_chunks) {
                free(chunks);
                return NULL;
            }
        ...
        zw_png_chunk_info *chunk = chunks + chunk_num;
        const uint8_t *chunk_data = data + offset;
        chunk->offset = offset;
        chunk->length = zw_swap_endian_uint32(*((uint32_t *)chunk_data));
        ...
        chunk->crc32 = zw_swap_endian_uint32(*((uint32_t *)(chunk_data + 8 + chunk->length)));
        chunk_num++;
        offset += 12 + chunk->length;
        
        switch (chunk->fourcc) {
            case ZW_FOUR_CC('a', 'c', 'T', 'L') : {
                if (chunk->length == 8) {
                    apng_frame_number = zw_swap_endian_uint32(*((uint32_t *)(chunk_data + 8)));
                    apng_loop_num = zw_swap_endian_uint32(*((uint32_t *)(chunk_data + 12)));
                } else {
                    apng_chunk_error = true;
                }
            } break;
            case ZW_FOUR_CC('f', 'c', 'T', 'L') :
            case ZW_FOUR_CC('f', 'd', 'A', 'T') : {
                if (chunk->fourcc == ZW_FOUR_CC('f', 'c', 'T', 'L')) {
                    if (chunk->length != 26) {
                        apng_chunk_error = true;
                    } else {
                        apng_frame_index++;
                ...
                if (chunk->length > 4) {
                    uint32_t sequence = zw_swap_endian_uint32(*((uint32_t *)(chunk_data + 8)));
    ...
    if (chunk_num < 3 ||
        chunks->fourcc != ZW_FOUR_CC('I', 'H', 'D', 'R') ||
        chunks->length != 13) {
        free(chunks);
        return NULL;
    }
    ...
    // png info
    zw_png_info *info = calloc(1, sizeof(zw_png_info));
    if (!info) {
        free(chunks);
        return NULL;
    }
    ...
    zw_png_chunk_IHDR_read(&info->header, data + chunks->offset + 8);
    ...
    if (!apng_chunk_error && apng_frame_number == apng_frame_index && apng_frame_number >= 1) {
        ...
        if (!zw_png_validate_animation_chunk_order(info->chunks, info->chunk_num, &first_IDAT_index, &first_frame_is_cover)) {
        ...
        info->apng_frames = calloc(apng_frame_number, sizeof(zw_png_frame_info));
        if (!info->apng_frames) {
            zw_png_info_release(info);
            return NULL;
        }
        ...
        for (int32_t i = 0; i < info->chunk_num; i++) {
            zw_png_chunk_info *chunk = info->chunks + i;
            switch (chunk->fourcc) {
                case ZW_FOUR_CC('I', 'D', 'A', 'T'): {
                    if (info->apng_shared_insert_index == 0) {
                        info->apng_shared_insert_index = i;
                    }
                    if (first_frame_is_cover) {
                        zw_png_frame_info *frame = info->apng_frames + frame_index;
                        frame->chunk_num++;
                ...
                case ZW_FOUR_CC('f', 'c', 'T', 'L'): {
                    ...
                    zw_png_chunk_fcTL_read(&frame->frame_control, data + chunk->offset + 8);
                } break;
                case ZW_FOUR_CC('f', 'd', 'A', 'T'): {
                    zw_png_frame_info *frame = info->apng_frames + frame_index;
                    frame->chunk_num++;
...
/**
 ...
 Returns NULL if an error occurs.
 */
static uint8_t *zw_png_copy_frame_data_at_index(const uint8_t *data,
                                                const zw_png_info *info,
                                                const uint32_t index,
                                                uint32_t *size) {
    if (index >= info->apng_frame_num) return NULL;
    
    zw_png_frame_info *frame_info = info->apng_frames + index;
    ...
    *size = frame_remux_size;
    ...
    for (uint32_t i = 0; i < info->apng_shared_chunk_num; i++) {
        ...
        zw_png_chunk_info *shared_chunk_info = info->chunks + shared_chunk_index;
        ...
        if (shared_chunk_index >= info->apng_shared_insert_index && !inserted) { // replace IDAT with fdAT
            ...
            for (uint32_t c = 0; c < frame_info->chunk_num; c++) {
                zw_png_chunk_info *insert_chunk_info = info->chunks + frame_info->chunk_index + c;
                if (insert_chunk_info->fourcc == ZW_FOUR_CC('f', 'd', 'A', 'T')) {
                    *((uint32_t *)(frame_data + data_offset)) = zw_swap_endian_uint32(insert_chunk_info->length - 4);
                    *((uint32_t *)(frame_data + data_offset + 4)) = ZW_FOUR_CC('I', 'D', 'A', 'T');
                    ...
                    *((uint32_t *)(frame_data + data_offset + insert_chunk_info->length + 4)) = zw_swap_endian_uint32(crc);
        ...
        if (shared_chunk_info->fourcc == ZW_FOUR_CC('I', 'H', 'D', 'R')) {
            uint8_t tmp[25] = {0};
            memcpy(tmp, data + shared_chunk_info->offset, 25);
            zw_png_chunk_IHDR IHDR = info->header;
            IHDR.width = frame_info->frame_control.width;
            IHDR.height = frame_info->frame_control.height;
            zw_png_chunk_IHDR_write(&IHDR, tmp + 8);
            *((uint32_t *)(tmp + 21)) = zw_swap_endian_uint32((uint32_t)crc32(0, tmp + 4, 17));
...
static inline size_t ZWImageByteAlign(size_t size, size_t alignment) {
    return ((size + (alignment - 1)) / alignment) * alignment;
}

/// Convert degree to radians
static inline CGFloat ZWImageDegreesToRadians(CGFloat degrees) {
    return degrees * M_PI / 180;
}

CGColorSpaceRef ZWCGColorSpaceGetDeviceRGB() {
...
CGColorSpaceRef ZWCGColorSpaceGetDeviceGray() {
    static CGColorSpaceRef space;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
BOOL ZWCGColorSpaceIsDeviceRGB(CGColorSpaceRef space) {
    return space && CFEqual(space, ZWCGColorSpaceGetDeviceRGB());
}

BOOL ZWCGColorSpaceIsDeviceGray(CGColorSpaceRef space) {
    return space && CFEqual(space, ZWCGColorSpaceGetDeviceGray());
}
 ...
 */
static void ZWCGDataProviderReleaseDataCallback(void *info, const void *data, size_t size) {
    if (info) free(info);
}
 ...
 */
static BOOL ZWCGImageDecodeToBitmapBufferWithAnyFormat(CGImageRef srcImage, vImage_Buffer *dest, vImage_CGImageFormat *destFormat) {
...
/**
 ...
 @return Whether succeed.
 */
static BOOL ZWCGImageDecodeToBitmapBufferWith32BitFormat(CGImageRef srcImage, vImage_Buffer *dest, CGBitmapInfo bitmapInfo) {
    ...
    destFormat.colorSpace = ZWCGColorSpaceGetDeviceRGB();
    destFormat.bitmapInfo = bitmapInfo;
    dest->data = NULL;
    if (ZWCGImageDecodeToBitmapBufferWithAnyFormat(srcImage, dest, &destFormat)) return YES;
    ...
    CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, ZWCGColorSpaceGetDeviceRGB(), contextBitmapInfo);
...
CGImageRef ZWCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay) {
    ...
    if (decodeForDisplay) { //decode with redraw (may lose some precision)
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, ZWCGColorSpaceGetDeviceRGB(), bitmapInfo);
...
CGImageRef YYCGImageCreateAffineTransformCopy(CGImageRef imageRef, CGAffineTransform transform, CGSize destSize, CGBitmapInfo destBitmapInfo) {
    ...
    if(!ZWCGImageDecodeToBitmapBufferWith32BitFormat(imageRef, &src, kCGImageAlphaFirst | kCGBitmapByteOrderDefault)) return NULL;
    
    size_t destBytesPerRow = ZWImageByteAlign(destWidth * 4, 32);
    ...
    tmpProvider = CGDataProviderCreateWithData(tmp.data, tmp.data, destHeight * destBytesPerRow, ZWCGDataProviderReleaseDataCallback);
    if (!tmpProvider) goto fail;
    tmp.data = NULL; // hold by provider
    tmpImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, ZWCGColorSpaceGetDeviceRGB(), kCGImageAlphaFirst | kCGBitmapByteOrderDefault, tmpProvider, NULL, false, kCGRenderingIntentDefault);
    ...
    if (!ZWCGImageDecodeToBitmapBufferWith32BitFormat(tmpImage, &dest, destBitmapInfo)) goto fail;
    CFRelease(tmpImage);
    tmpImage = NULL;
    
    destProvider = CGDataProviderCreateWithData(dest.data, dest.data, destHeight * destBytesPerRow, ZWCGDataProviderReleaseDataCallback);
    if (!destProvider) goto fail;
    dest.data = NULL; // hold by provider
    destImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, ZWCGColorSpaceGetDeviceRGB(), destBitmapInfo, destProvider, NULL, false, kCGRenderingIntentDefault);
...
CGImageRef ZWCGImageCreateCopyWithOrientation(CGImageRef imageRef, UIImageOrientation orientation, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    switch (orientation) {
        case UIImageOrientationDown: {
            transform = CGAffineTransformMakeRotation(ZWImageDegreesToRadians(180));
            transform = CGAffineTransformTranslate(transform, -(CGFloat)width, -(CGFloat)height);
        } break;
        case UIImageOrientationLeft: {
            transform = CGAffineTransformMakeRotation(ZWImageDegreesToRadians(90));
...
ZWImageType YYImageDetectType(CFDataRef data) {
    if (!data) return ZWImageTypeUnknown;
    uint64_t length = CFDataGetLength(data);
    if (length < 16) return ZWImageTypeUnknown;
    
    const char *bytes = (char *)CFDataGetBytePtr(data);
    
    uint32_t magic4 = *((uint32_t *)bytes);
    switch (magic4) {
        case ZW_FOUR_CC(0x4D, 0x4D, 0x00, 0x2A): { // big endian TIFF
            return ZWImageTypeTIFF;
        } break;
            
        case ZW_FOUR_CC(0x49, 0x49, 0x2A, 0x00): { // little endian TIFF
            return ZWImageTypeTIFF;
        } break;
            
        case ZW_FOUR_CC(0x00, 0x00, 0x01, 0x00): { // ICO
            return ZWImageTypeICO;
        } break;
            
        case ZW_FOUR_CC(0x00, 0x00, 0x02, 0x00): { // CUR
            return ZWImageTypeICO;
        } break;
            
        case ZW_FOUR_CC('i', 'c', 'n', 's'): { // ICNS
            return ZWImageTypeICNS;
        } break;
            
        case ZW_FOUR_CC('G', 'I', 'F', '8'): { // GIF
            return ZWImageTypeGIF;
        } break;
            
        case ZW_FOUR_CC(0x89, 'P', 'N', 'G'): {  // PNG
            uint32_t tmp = *((uint32_t *)(bytes + 4));
            if (tmp == ZW_FOUR_CC('\r', '\n', 0x1A, '\n')) {
                return ZWImageTypePNG;
            }
        } break;
            
        case ZW_FOUR_CC('R', 'I', 'F', 'F'): { // WebP
            uint32_t tmp = *((uint32_t *)(bytes + 8));
            if (tmp == ZW_FOUR_CC('W', 'E', 'B', 'P')) {
                return ZWImageTypeWebP;
            }
        } break;
            /*
    ...
    switch (magic2) {
        ...
        case ZW_TWO_CC('P', 'I'):
        case ZW_TWO_CC('C', 'I'):
        case ZW_TWO_CC('C', 'P'): { // BMP
            return ZWImageTypeBMP;
        }
        case ZW_TWO_CC(0xFF, 0x4F): { // JPEG2000
            return ZWImageTypeJPEG2000;
        }
    }
    
    // JPG             FF D8 FF
    if (memcmp(bytes,"\377\330\377",3) == 0) return ZWImageTypeJPEG;
    
    // JP2
    if (memcmp(bytes + 4, "\152\120\040\040\015", 5) == 0) return ZWImageTypeJPEG2000;
    
    return ZWImageTypeUnknown;
}

CFStringRef ZWImageTypeToUTType(ZWImageType type) {
    switch (type) {
        case ZWImageTypeJPEG: return kUTTypeJPEG;
        case ZWImageTypeJPEG2000: return kUTTypeJPEG2000;
        case ZWImageTypeTIFF: return kUTTypeTIFF;
        case ZWImageTypeBMP: return kUTTypeBMP;
        case ZWImageTypeICO: return kUTTypeICO;
        case ZWImageTypeICNS: return kUTTypeAppleICNS;
        case ZWImageTypeGIF: return kUTTypeGIF;
        case ZWImageTypePNG: return kUTTypePNG;
        default: return NULL;
    }
}

ZWImageType ZWImageTypeFromUTType(CFStringRef uti) {
    static NSDictionary *dic;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dic = @{(id)kUTTypeJPEG : @(ZWImageTypeJPEG),
                (id)kUTTypeJPEG2000 : @(ZWImageTypeJPEG2000),
                (id)kUTTypeTIFF : @(ZWImageTypeTIFF),
                (id)kUTTypeBMP : @(ZWImageTypeBMP),
                (id)kUTTypeICO : @(ZWImageTypeICO),
                (id)kUTTypeAppleICNS : @(ZWImageTypeICNS),
                (id)kUTTypeGIF : @(ZWImageTypeGIF),
                (id)kUTTypePNG : @(ZWImageTypePNG)};
    });
    if (!uti) return ZWImageTypeUnknown;
    NSNumber *num = dic[(__bridge __strong id)(uti)];
    return num.unsignedIntegerValue;
}

NSString *ZWImageTypeGetExtension(ZWImageType type) {
    switch (type) {
        case ZWImageTypeJPEG: return @"jpg";
        case ZWImageTypeJPEG2000: return @"jp2";
        case ZWImageTypeTIFF: return @"tiff";
        case ZWImageTypeBMP: return @"bmp";
        case ZWImageTypeICO: return @"ico";
        case ZWImageTypeICNS: return @"icns";
        case ZWImageTypeGIF: return @"gif";
        case ZWImageTypePNG: return @"png";
        case ZWImageTypeWebP: return @"webp";
        default: return nil;
    }
}

CFDataRef ZWCGImageCreateEncodedData(CGImageRef imageRef, ZWImageType type, CGFloat quality) {
    if (!imageRef) return nil;
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    
    if (type == ZWImageTypeWebP) {
    ...
    CFStringRef uti = ZWImageTypeToUTType(type);
    if (!uti) return nil;
...
#if ZWIMAGE_WEBP_ENABLED

BOOL ZWImageWebPAvailable() {
    return YES;
}

CFDataRef ZWCGImageCreateEncodedWebPData(CGImageRef imageRef, BOOL lossless, CGFloat quality, int compressLevel, ZWImagePreset preset) {
    if (!imageRef) return nil;
    size_t width = CGImageGetWidth(imageRef);
    size_t height = CGImageGetHeight(imageRef);
    ...
    if(!ZWCGImageDecodeToBitmapBufferWith32BitFormat(imageRef, &buffer, kCGImageAlphaLast | kCGBitmapByteOrderDefault)) return nil;
...
@implementation ZWImageFrame
+ (instancetype)frameWithImage:(UIImage *)image {
    ZWImageFrame *frame = [self new];
    frame.image = image;
    return frame;
}
- (id)copyWithZone:(NSZone *)zone {
    ZWImageFrame *frame = [self.class new];
    frame.index = _index;
    frame.width = _width;
...
@interface _ZWImageDecoderFrame : ZWImageFrame
...
@implementation _ZWImageDecoderFrame
- (id)copyWithZone:(NSZone *)zone {
    _ZWImageDecoderFrame *frame = [super copyWithZone:zone];
...
- (void)dealloc {
    ...
    if (_apngSource) zw_png_info_release(_apngSource);
#if YYIMAGE_WEBP_ENABLED
...
+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    ZWImageDecoder *decoder = [[ZWImageDecoder alloc] initWithScale:scale];
...
- (ZWImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    ZWImageFrame *result = nil;
    pthread_mutex_lock(&_lock);
...
- (NSTimeInterval)frameDurationAtIndex:(NSUInteger)index {
    ...
    if (index < _frames.count) {
        result = ((_ZWImageDecoderFrame *)_frames[index]).duration;
    }
...
- (BOOL)_updateData:(NSData *)data final:(BOOL)final {
    ...
    ZWImageType type = YYImageDetectType((__bridge CFDataRef)data);
...
- (ZWImageFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    if (index >= _frames.count) return 0;
    _ZWImageDecoderFrame *frame = [(_ZWImageDecoderFrame *)_frames[index] copy];
    BOOL decoded = NO;
    BOOL extendToCanvas = NO;
    if (_type != ZWImageTypeICO && decodeForDisplay) { // ICO contains multi-size frame and should not extend to canvas.
        extendToCanvas = YES;
    }
    ...
    if (!_needBlend) {
        ...
        if (decodeForDisplay && !decoded) {
            CGImageRef imageRefDecoded = ZWCGImageCreateDecodedCopy(imageRef, YES);
            if (imageRefDecoded) {
                CFRelease(imageRef);
    ...
    if (_blendFrameIndex + 1 == frame.index) {
        ...
        if (frame.blendFromIndex == frame.index) {
            ...
            if (frame.dispose == ZWImageDisposeBackground) {
    ...
    if (extendToCanvas) {
        ...
        frame.offsetX = 0;
        frame.offsetY = 0;
        frame.dispose = ZWImageDisposeNone;
        frame.blend = ZWImageBlendNone;
    }
    return frame;
}
...
- (void)_updateSourceAPNG {
    ...
    zw_png_info_release(_apngSource);
    _apngSource = nil;
    ...
    for (uint32_t i = 0; i < apng->apng_frame_num; i++) {
        _ZWImageDecoderFrame *frame = [_ZWImageDecoderFrame new];
        [frames addObject:frame];
        
        zw_png_frame_info *fi = apng->apng_frames + i;
        frame.index = i;
        frame.duration = zw_png_delay_to_seconds(fi->frame_control.delay_num, fi->frame_control.delay_den);
        frame.hasAlpha = YES;
        ...
        switch (fi->frame_control.dispose_op) {
            case ZW_PNG_DISPOSE_OP_BACKGROUND: {
                frame.dispose = ZWImageDisposeBackground;
            } break;
            case ZW_PNG_DISPOSE_OP_PREVIOUS: {
                frame.dispose = ZWImageDisposePrevious;
            } break;
            default: {
                frame.dispose = ZWImageDisposeNone;
            } break;
        ...
        switch (fi->frame_control.blend_op) {
            case ZW_PNG_BLEND_OP_OVER: {
                frame.blend = ZWImageBlendOver;
            } break;
        ...
        if (frame.blend == ZWImageBlendNone && frame.isFullSize) {
            frame.blendFromIndex  = i;
            if (frame.dispose != ZWImageDisposePrevious) lastBlendIndex = i;
        } else {
            if (frame.dispose == ZWImageDisposeBackground && frame.isFullSize) {
                frame.blendFromIndex = lastBlendIndex;
                lastBlendIndex = i + 1;
            } else {
                frame.blendFromIndex = lastBlendIndex;
...
- (void)_updateSourceImageIO {
    ...
    if (!_finalized) { // ignore multi-frame before finalized
        ...
        if (_type == ZWImageTypePNG) { // use custom apng decoder and ignore multi-frame
            _frameCount = 1;
        }
        if (_type == ZWImageTypeGIF) { // get gif loop count
            CFDictionaryRef properties = CGImageSourceCopyProperties(_source, NULL);
            if (properties) {
...
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
    ...
    _ZWImageDecoderFrame *frame = _frames[index];
    ...
    if (_source) {
        ...
        if (imageRef && extendToCanvas) {
            ...
            if (width == _width && height == _height) {
                CGImageRef imageRefExtended = ZWCGImageCreateDecodedCopy(imageRef, YES);
                if (imageRefExtended) {
                    CFRelease(imageRef);
                ...
                CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, ZWCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
                if (context) {
    ...
    if (_apngSource) {
        uint32_t size = 0;
        uint8_t *bytes = zw_png_copy_frame_data_at_index(_data.bytes, _apngSource, (uint32_t)index, &size);
        if (!bytes) return NULL;
        CGDataProviderRef provider = CGDataProviderCreateWithData(bytes, bytes, size, ZWCGDataProviderReleaseDataCallback);
        if (!provider) {
            free(bytes);
            return NULL;
        }
        ...
        if (!imageRef) return NULL;
        if (extendToCanvas) {
            CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, ZWCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst); //bgrA
            if (context) {
    ...
    if (_webpSource) {
        ...
        CGImageRef image = CGImageCreate(width, height, bitsPerComponent, bitsPerPixel, bytesPerRow, ZWCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
        CFRelease(provider);
        if (decoded) *decoded = YES;
        return image;
    }
...
- (BOOL)_createBlendContextIfNeeded {
    if (!_blendCanvas) {
        _blendFrameIndex = NSNotFound;
        _blendCanvas = CGBitmapContextCreate(NULL, _width, _height, 8, 0, ZWCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    BOOL suc = _blendCanvas != NULL;
    return suc;
}

- (void)_blendImageWithFrame:(_ZWImageDecoderFrame *)frame {
    if (frame.dispose == ZWImageDisposePrevious) {
        // nothing
    } else if (frame.dispose == ZWImageDisposeBackground) {
        CGContextClearRect(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height));
    } else { // no dispose
        if (frame.blend == ZWImageBlendOver) {
...
- (CGImageRef)_newBlendedImageWithFrame:(_ZWImageDecoderFrame *)frame CF_RETURNS_RETAINED{
    CGImageRef imageRef = NULL;
    if (frame.dispose == ZWImageDisposePrevious) {
        if (frame.blend == ZWImageBlendOver) {
...
- (instancetype)init {
    ...
    return [self initWithType:ZWImageTypeUnknown];
}

- (instancetype)initWithType:(ZWImageType)type {
    if (type == ZWImageTypeUnknown || type >= ZWImageTypeOther) {
    ...
    switch (type) {
        case ZWImageTypeJPEG:
        case ZWImageTypeJPEG2000: {
            _quality = 0.9;
        } break;
        case ZWImageTypeTIFF:
        case ZWImageTypeBMP:
        case ZWImageTypeGIF:
        case ZWImageTypeICO:
        case ZWImageTypeICNS:
        case ZWImageTypePNG: {
            _quality = 1;
            _lossless = YES;
        } break;
...
- (CGImageDestinationRef)_newImageDestination:(id)dest imageCount:(NSUInteger)count CF_RETURNS_RETAINED {
    ...
    if ([dest isKindOfClass:[NSString class]]) {
        ...
        if (url) {
            destination = CGImageDestinationCreateWithURL((CFURLRef)url, ZWImageTypeToUTType(_type), count, NULL);
        }
    } else if ([dest isKindOfClass:[NSMutableData class]]) {
        destination = CGImageDestinationCreateWithData((CFMutableDataRef)dest, ZWImageTypeToUTType(_type), count, NULL);
    }
    return destination;
}
...
- (void)_encodeImageWithDestination:(CGImageDestinationRef)destination imageCount:(NSUInteger)count {
    ...
    for (int i = 0; i < count; i++) {
        @autoreleasepool {
            ...
            if (_type == ZWImageTypeGIF && count > 1) {
            ...
            if ([imageSrc isKindOfClass:[UIImage class]]) {
                ...
                if (image.imageOrientation != UIImageOrientationUp && image.CGImage) {
                    ...
                    CGImageRef rotated = ZWCGImageCreateCopyWithOrientation(image.CGImage, image.imageOrientation, info);
                    if (rotated) {
...
- (CGImageRef)_newCGImageFromIndex:(NSUInteger)index decoded:(BOOL)decoded CF_RETURNS_RETAINED {
    ...
    if (image.imageOrientation != UIImageOrientationUp) {
        return ZWCGImageCreateCopyWithOrientation(imageRef, image.imageOrientation, kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    if (decoded) {
        return ZWCGImageCreateDecodedCopy(imageRef, YES);
    }
...
- (NSData *)_encodeWithImageIO {
    ...
    NSUInteger count = _type == ZWImageTypeGIF ? _images.count : 1;
...
- (NSData *)_encodeAPNG {
    ...
    for (int i = 0; i < _images.count; i++) {
        ...
        CFDataRef frameData = ZWCGImageCreateEncodedData(decoded, ZWImageTypePNG, 1);
        CFRelease(decoded);
    ...
    if (firstFrameSize.width < canvasWidth || firstFrameSize.height < canvasHeight) {
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8,
                                                     0, ZWCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
        if (!context) {
            CFRelease(decoded);
            return nil;
        }
        ...
        CFDataRef frameData = ZWCGImageCreateEncodedData(extendedImage, ZWImageTypePNG, 1);
        if (!frameData) {
    ...
    zw_png_info *info = zw_png_info_create(firstFrameData.bytes, (uint32_t)firstFrameData.length);
    if (!info) return nil;
    ...
    png_header[0] = ZW_FOUR_CC(0x89, 0x50, 0x4E, 0x47);
    png_header[1] = ZW_FOUR_CC(0x0D, 0x0A, 0x1A, 0x0A);
    ...
    for (int i = 0; i < info->chunk_num; i++) {
        ...
        if (!insertBefore && chunk->fourcc == ZW_FOUR_CC('I', 'D', 'A', 'T')) {
            insertBefore = YES;
            // insert acTL (APNG Control)
            uint32_t acTL[5] = {0};
            acTL[0] = zw_swap_endian_uint32(8); //length
            acTL[1] = ZW_FOUR_CC('a', 'c', 'T', 'L'); // fourcc
            acTL[2] = zw_swap_endian_uint32((uint32_t)pngDatas.count); // num frames
            acTL[3] = zw_swap_endian_uint32((uint32_t)_loopCount); // num plays
            acTL[4] = zw_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(acTL + 1), 12)); //crc32
            [result appendBytes:acTL length:20];
            
            // insert fcTL (first frame control)
            zw_png_chunk_fcTL chunk_fcTL = {0};
            ...
            zw_png_delay_to_fraction([(NSNumber *)_durations[0] doubleValue], &chunk_fcTL.delay_num, &chunk_fcTL.delay_den);
            chunk_fcTL.delay_num = chunk_fcTL.delay_num;
            chunk_fcTL.delay_den = chunk_fcTL.delay_den;
            chunk_fcTL.dispose_op = ZW_PNG_DISPOSE_OP_BACKGROUND;
            chunk_fcTL.blend_op = ZW_PNG_BLEND_OP_SOURCE;
            
            uint8_t fcTL[38] = {0};
            *((uint32_t *)fcTL) = zw_swap_endian_uint32(26); //length
            *((uint32_t *)(fcTL + 4)) = ZW_FOUR_CC('f', 'c', 'T', 'L'); // fourcc
            zw_png_chunk_fcTL_write(&chunk_fcTL, fcTL + 8);
            *((uint32_t *)(fcTL + 34)) = zw_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(fcTL + 4), 30));
            [result appendBytes:fcTL length:38];
            
            apngSequenceIndex++;
        }
        
        if (!insertAfter && insertBefore && chunk->fourcc != ZW_FOUR_CC('I', 'D', 'A', 'T')) {
            insertAfter = YES;
            // insert fcTL and fdAT (APNG frame control and data)
            
            for (int i = 1; i < pngDatas.count; i++) {
                NSData *frameData = pngDatas[i];
                zw_png_info *frame = zw_png_info_create(frameData.bytes, (uint32_t)frameData.length);
                if (!frame) {
                    zw_png_info_release(info);
                    return nil;
                }
                ...
                zw_png_delay_to_fraction([(NSNumber *)_durations[i] doubleValue], &chunk_fcTL.delay_num, &chunk_fcTL.delay_den);
                chunk_fcTL.delay_num = chunk_fcTL.delay_num;
                chunk_fcTL.delay_den = chunk_fcTL.delay_den;
                chunk_fcTL.dispose_op = ZW_PNG_DISPOSE_OP_BACKGROUND;
                chunk_fcTL.blend_op = ZW_PNG_BLEND_OP_SOURCE;
                ...
                *((uint32_t *)(fcTL + 34)) = zw_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(fcTL + 4), 30));
                ...
                for (int d = 0; d < frame->chunk_num; d++) {
                    zw_png_chunk_info *dchunk = frame->chunks + d;
                    if (dchunk->fourcc == ZW_FOUR_CC('I', 'D', 'A', 'T')) {
                        uint32_t length = zw_swap_endian_uint32(dchunk->length + 4);
                        [result appendBytes:&length length:4]; //length
                        uint32_t fourcc = ZW_FOUR_CC('f', 'd', 'A', 'T');
                        [result appendBytes:&fourcc length:4]; //fourcc
                        uint32_t sq = zw_swap_endian_uint32(apngSequenceIndex);
                        ...
                        uint32_t crc = zw_swap_endian_uint32((uint32_t)crc32(0, bytes, dchunk->length + 8));
                        [result appendBytes:&crc length:4]; //crc
                        
                        apngSequenceIndex++;
                    }
                }
                zw_png_info_release(frame);
            }
        }
...
- (NSData *)encode {
    ...
    if (_type == ZWImageTypePNG) return [self _encodeAPNG];
    if (_type == ZWImageTypeWebP) return [self _encodeWebP];
    return nil;
}
...
+ (NSData *)encodeImage:(UIImage *)image type:(ZWImageType)type quality:(CGFloat)quality {
    ZWImageEncoder *encoder = [[ZWImageEncoder alloc] initWithType:type];
    encoder.quality = quality;
    [encoder addImage:image duration:0];
    return [encoder encode];
}

+ (NSData *)encodeImageWithDecoder:(ZWImageDecoder *)decoder type:(ZWImageType)type quality:(CGFloat)quality {
...
@implementation UIImage (ZWImageCoder)

- (instancetype)imageByDecoded {
    if (self.isDecodedForDisplay) return self;
    CGImageRef imageRef = self.CGImage;
    if (!imageRef) return self;
    CGImageRef newImageRef = ZWCGImageCreateDecodedCopy(imageRef, YES);
...
- (NSData *)_imageDataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[ZWImage class]]) {
        ZWImage *image = (id)self;
        if (image.animatedImageData) {
            if (forSystem) { // system only support GIF and PNG
                if (image.animatedImageType == ZWImageTypeGIF ||
                    image.animatedImageType == ZWImageTypePNG) {
    ...
    if (!data) {
        ...
        if (imageRef) {
            ...
            if (self.imageOrientation != UIImageOrientationUp) {
                CGImageRef rotated = ZWCGImageCreateCopyWithOrientation(imageRef, self.imageOrientation, bitmapInfo | alphaInfo);
                if (rotated) {
                    CFRelease(imageRef);
                    imageRef = rotated;
                }
