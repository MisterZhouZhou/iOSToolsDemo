...
typedef NS_ENUM(NSUInteger, ZWAnimatedImageType) {
    ZWAnimatedImageTypeNone = 0,
    ZWAnimatedImageTypeImage,
    ZWAnimatedImageTypeHighlightedImage,
    ZWAnimatedImageTypeImages,
    ZWAnimatedImageTypeHighlightedImages,
};

@interface ZWAnimatedImageView() {
    @package
    UIImage <ZWAnimatedImage> *_curAnimatedImage;
    // dispatch_semaphore_t执行效率仅次于OSSpinLock
...
@interface _ZWAnimatedImageViewFetchOperation : NSOperation
@property (nonatomic, weak) ZWAnimatedImageView *view;
@property (nonatomic, assign) NSUInteger nextIndex;
@property (nonatomic, strong) UIImage <ZWAnimatedImage> *curImage;
@end

@implementation _ZWAnimatedImageViewFetchOperation
- (void)main {
    __strong ZWAnimatedImageView *view = _view;
    if (!view) return;
...
- (void)resetAnimated {
    if (!_link) {
        ...
        _link = [CADisplayLink displayLinkWithTarget:[ZWWeakProxy proxyWithTarget:self] selector:@selector(step:)];
        if (_runloopMode) {
...
- (void)setImage:(UIImage *)image {
    ...
    [self setImage:image withType:ZWAnimatedImageTypeImage];
}

- (void)setHighlightedImage:(UIImage *)highlightedImage {
    if (self.highlightedImage == highlightedImage) return;
    [self setImage:highlightedImage withType:ZWAnimatedImageTypeHighlightedImage];
}

- (void)setAnimationImages:(NSArray *)animationImages {
    if (self.animationImages == animationImages) return;
    [self setImage:animationImages withType:ZWAnimatedImageTypeImages];
}
...
- (void)setHighlightedAnimationImages:(NSArray *)highlightedAnimationImages {
    ...
    [self setImage:highlightedAnimationImages withType:ZWAnimatedImageTypeHighlightedImages];
}
...
- (id)imageForType:(ZWAnimatedImageType)type {
    switch (type) {
        case ZWAnimatedImageTypeNone: return nil;
        case ZWAnimatedImageTypeImage: return self.image;
        case ZWAnimatedImageTypeHighlightedImage: return self.highlightedImage;
        case ZWAnimatedImageTypeImages: return self.animationImages;
        case ZWAnimatedImageTypeHighlightedImages: return self.highlightedAnimationImages;
    }
    return nil;
}

- (ZWAnimatedImageType)currentImageType {
    ZWAnimatedImageType curType = ZWAnimatedImageTypeNone;
    if (self.highlighted) {
        if (self.highlightedAnimationImages.count) curType = ZWAnimatedImageTypeHighlightedImages;
        else if (self.highlightedImage) curType = ZWAnimatedImageTypeHighlightedImage;
    }
    if (curType == ZWAnimatedImageTypeNone) {
        if (self.animationImages.count) curType = ZWAnimatedImageTypeImages;
        else if (self.image) curType = ZWAnimatedImageTypeImage;
    }
    return curType;
}

- (void)setImage:(id)image withType:(ZWAnimatedImageType)type {
    [self stopAnimating];
    if (_link) [self resetAnimated];
    _curFrame = nil;
    switch (type) {
        case ZWAnimatedImageTypeNone: break;
        case ZWAnimatedImageTypeImage: super.image = image; break;
        case ZWAnimatedImageTypeHighlightedImage: super.highlightedImage = image; break;
        case ZWAnimatedImageTypeImages: super.animationImages = image; break;
        case ZWAnimatedImageTypeHighlightedImages: super.highlightedAnimationImages = image; break;
    }
    [self imageChanged];
}

- (void)imageChanged {
    ZWAnimatedImageType newType = [self currentImageType];
    id newVisibleImage = [self imageForType:newType];
    NSUInteger newImageFrameCount = 0;
    BOOL hasContentsRect = NO;
    if ([newVisibleImage isKindOfClass:[UIImage class]] &&
        [newVisibleImage conformsToProtocol:@protocol(ZWAnimatedImage)]) {
        newImageFrameCount = ((UIImage<ZWAnimatedImage> *) newVisibleImage).animatedImageFrameCount;
        if (newImageFrameCount > 1) {
            hasContentsRect = [((UIImage<ZWAnimatedImage> *) newVisibleImage) respondsToSelector:@selector(animatedImageContentsRectAtIndex:)];
        }
    }
    ...
    if (hasContentsRect) {
        CGRect rect = [((UIImage<ZWAnimatedImage> *) newVisibleImage) animatedImageContentsRectAtIndex:0];
...
- (void)calcMaxBufferCount {
    ...
    maxBufferCount = ZW_CLAMP(maxBufferCount, 1, 512);
...
- (void)startAnimating {
    ZWAnimatedImageType type = [self currentImageType];
    if (type == ZWAnimatedImageTypeImages || type == ZWAnimatedImageTypeHighlightedImages) {
...
- (void)step:(CADisplayLink *)link {
    UIImage <ZWAnimatedImage> *image = _curAnimatedImage;
    ...
    if (!bufferIsFull && _requestQueue.operationCount == 0) { // if some work not finished, wait for next opportunity
        _ZWAnimatedImageViewFetchOperation *operation = [_ZWAnimatedImageViewFetchOperation new];
        operation.view = self;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    UIImage *image = [aDecoder decodeObjectForKey:@"ZWAnimatedImage"];
    UIImage *highlightedImage = [aDecoder decodeObjectForKey:@"ZWHighlightedAnimatedImage"];
    if (image) {
        self.image = image;
        [self setImage:image withType:ZWAnimatedImageTypeImage];
    }
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    // conformsToProtocol查看其包含其父级是否实现了协议
    BOOL ani, multi;
    ani = [self.image conformsToProtocol:@protocol(ZWAnimatedImage)];
    multi = (ani && ((UIImage <ZWAnimatedImage> *)self.image).animatedImageFrameCount > 1);
    if (multi) [aCoder encodeObject:self.image forKey:@"ZWAnimatedImage"];
    
    ani = [self.highlightedImage conformsToProtocol:@protocol(ZWAnimatedImage)];
    multi = (ani && ((UIImage <ZWAnimatedImage> *)self.highlightedImage).animatedImageFrameCount > 1);
    if (multi) [aCoder encodeObject:self.highlightedImage forKey:@"ZWHighlightedAnimatedImage"];
}
@end
