...
CGContextRef ZWCGContextCreateARGBBitmapContext(CGSize size, BOOL opaque, CGFloat scale) {
...
CGContextRef ZWCGContextCreateGrayBitmapContext(CGSize size, CGFloat scale) {
    size_t width = ceil(size.width * scale);
    size_t height = ceil(size.height * scale);
    if (width < 1 || height < 1) return NULL;
...
#pragma mark - 获取屏幕的大小，height>width
CGSize ZWScreenSize() {
    static CGSize size;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
CGAffineTransform ZWCGAffineTransformGetFromPoints(CGPoint before[3], CGPoint after[3]) {
...
CGAffineTransform YYCGAffineTransformGetFromViews(UIView *from, UIView *to) {
    ...
    return ZWCGAffineTransformGetFromPoints(before, after);
}
