//
//  NSDictionary+ZWAdd.m
//  OCToolsDemo
//
//  Created by chengfei on 2017/12/4.
...
ZWSYNTH_DUMMY_CLASS(NSDictionary_ZWAdd)


@interface _ZWXMLDictionaryParser : NSObject <NSXMLParserDelegate>
@end

@implementation _ZWXMLDictionaryParser {
    NSMutableDictionary *_root;  // 根节点
...
@implementation NSDictionary (ZWAdd)
...
#pragma mark - 过滤出在数据源中与keys中key对应的字典
...
+ (NSDictionary *)dictionaryWithXML:(id)xml {
    _ZWXMLDictionaryParser *parser = nil;
    if ([xml isKindOfClass:[NSString class]]) {
        parser = [[_ZWXMLDictionaryParser alloc] initWithString:xml];
    } else if ([xml isKindOfClass:[NSData class]]) {
        parser = [[_ZWXMLDictionaryParser alloc] initWithData:xml];
    }
...
@implementation NSMutableDictionary (ZWAdd)
