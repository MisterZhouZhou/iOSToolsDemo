...
ZWSYNTH_DUMMY_CLASS(UIApplication_ZWAdd)

#define kNetworkIndicatorDelay (1/30.0)
@interface _ZWUIApplicationNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end

@implementation _ZWUIApplicationNetworkIndicatorInfo
@end

@implementation UIApplication (ZWAdd)
...
- (BOOL)isPirated {
    ...
    if (![self _zw_fileExistInMainBundle:@"_CodeSignature"]) {
        return YES;
    }
    
    if (![self _zw_fileExistInMainBundle:@"SC_Info"]) {
        return YES;
    }
...
- (BOOL)_zw_fileExistInMainBundle:(NSString *)name {
...
ZWSYNTH_DYNAMIC_PROPERTY_OBJECT(networkActivityInfo, setNetworkActivityInfo, RETAIN_NONATOMIC, _ZWUIApplicationNetworkIndicatorInfo *);
...
- (void)_changeNetworkActivityCount:(NSInteger)delta {
    @synchronized(self){
        dispatch_async_on_main_queue(^{
            _ZWUIApplicationNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [_ZWUIApplicationNetworkIndicatorInfo new];
