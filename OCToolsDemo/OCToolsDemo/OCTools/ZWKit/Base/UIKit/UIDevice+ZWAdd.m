...
ZWSYNTH_DUMMY_CLASS(UIDevice_ZWAdd)


@implementation UIDevice (ZWAdd)
...
typedef struct {
    ...
    uint64_t awdl_in;
    uint64_t awdl_out;
} zw_net_interface_counter;

static uint64_t zw_net_counter_add(uint64_t counter, uint64_t bytes) {
...
static uint64_t zw_net_counter_get_by_type(zw_net_interface_counter *counter, ZWNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & ZWNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & ZWNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & ZWNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & ZWNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & ZWNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & ZWNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}

static zw_net_interface_counter zw_get_net_interface_counter() {
    ...
    zw_net_interface_counter counter = {0};
        ...
        while (cursor) {
            if (cursor->ifa_addr->sa_family == AF_LINK) {
                ...
                if (name) {
                    ...
                    counter_in = zw_net_counter_add(counter_in, data->ifi_ibytes);
                    sharedInCounters[name] = @(counter_in);
                    
                    uint64_t counter_out = ((NSNumber *)sharedOutCounters[name]).unsignedLongLongValue;
                    counter_out = zw_net_counter_add(counter_out, data->ifi_obytes);
...
- (uint64_t)getNetworkTrafficBytes:(ZWNetworkTrafficType)types {
    zw_net_interface_counter counter = zw_get_net_interface_counter();
    return zw_net_counter_get_by_type(&counter, types);
}
